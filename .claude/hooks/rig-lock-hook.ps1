...
STATE HYGIENE: a NEW lock RESETS what the last session left behind, so a test cannot fail on unrelated leftovers. Cleared per client instance: setting.xml (it carries StartLocalHost), userdata/saves/, logs, imgui.ini, a stale game.pid, BepInEx config (re-copied from the source install, then SavePathOverride re-applied, because the copy WIPES it), LogOutput.log*, cache/, inspector requests/ and snapshots/. On the server: the ScenarioRunner Scenario value, the scenariorunner and inspector drop files, data/setting.xml, stale server.pid/host.pid/control.cmd. KEPT: rig.json, instance manifests, provision stamps, seeded mods (staleness is reported, fix with -Provision -Force), the dedicated server's saves and mods, other server configs, the hard links. It is refused while the rig is in use (the lock is still granted, with a warning), never fires on re-asserting a lock you already hold, and `-Lock -KeepState` opts out loudly. IT RESETS BETWEEN SESSIONS ONLY: two unrelated tests under ONE lock get no reset between them, so release and re-take the lock when the subject changes. Shared per-user state (PlayerCookie-v2.xml, PlayerPrefs, Blueprints) is only REPORTED at -Unlock, never restored.
