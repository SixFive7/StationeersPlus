...
- 2026-07-18: re-confirmed the load-bearing ordering claim against the 0.2.6403.27689 decompile while implementing the PowerGridPlus burn-reason join handshake. `NetworkClient.ProcessJoinData` (decompile 213188) still runs `ProcessThings` -> `AtmosphericsManager.DeserializeOnJoin` (the LaunchPadBooster suffix position) -> `StructureNetwork.StructureNetworksOnFinishedLoad` inside the try block, closes the reader, and only later calls `GameManager.UpdateThingsOnGameStart()` (whose delegate fires per-Thing `Thing.OnFinishedLoad`, decompile 203855) and then `Rocket.OnFinishedLoad` (213239). So `DeserializeJoinSuffix` still runs after `ProcessThings` and BEFORE every per-Thing `OnFinishedLoad`, which also makes the suffix deserializer a valid place to clear per-world mod caches that a `Thing.OnFinishedLoad` hook would otherwise repopulate from a previous local world's stale state (the PowerGridPlus burn-reason stale-attach fix relies on exactly this ordering). Vanilla-side ordering only; LaunchPadBooster's transpile internals were not re-read this pass, so section stamps are unchanged.
