...
- Any `AllPrefabs` hash not claimed by either registry = "vanilla" (or a direct-injection mod that left no trace; treat as a residual bucket and log its count). **This residual is not always empty.** Confirmed case (61-mod set, 2026-06-21): the Mirrored Devices mod clones vanilla devices and registers the 18 `*Mirrored` variants by direct injection, so it shows up in `ModLoader.LoadedMods` with `Prefabs.Count == 0`, contributes nothing to either registry, and all 18 fall into the residual and are misclassified as vanilla by this method alone. A prefab-cloning mod like this can only be caught by fallback (a) the vanilla diff or (c) the `RegisterExisting` stack-trace hook.

This is one pass, no game restarts, and names the specific mod for every prefab that came through the two supported loaders. It is the recommended approach for *named* attribution, but it under-counts mods by exactly the direct-injection residual (see the Mirrored Devices case above); pair it with a vanilla-diff when a complete mod/vanilla split is required.
...
- 2026-06-21: CORRECTION to the same-day note below that claimed an "empty direct-injection residual" for the 61-mod set. A cross-check against an independent no-mods capture (a second ScenarioRunner `device-port-dump` run with Workshop mods disabled: 321 power/data devices, vs 378 with mods) showed 57 mod-introduced power/data devices, not 39. The 18-device gap is entirely the Mirrored Devices mod's `*Mirrored` variants (e.g. `StructurePressureRegulatorMirrored`, `StructureVolumePumpMirrored`, `StructureLogicSorterMirrored`): genuinely mod-introduced, but registered by direct injection. `Mirrored Devices` appears in `ModLoader.LoadedMods` with `Prefabs.Count == 0`, so the registry join attributes all 18 to "vanilla". The registry-read (primary) method therefore UNDER-counts mods by the direct-injection residual whenever a prefab-cloning mod is active; the complete mod/vanilla split needs the two-pass vanilla diff (fallback a) or the `RegisterExisting` hook (fallback c). The earlier "empty residual" claim was made without a vanilla baseline and is withdrawn. Method/strategy text elsewhere on the page (which already lists direct injection as a residual case) is unaffected.
...
- Whether any shipped StationeersMods-only mod (no LaunchPadBooster dependency) registers prefabs by a route that leaves neither a `Mod.AllMods` nor a `LoadedMod.Prefabs` trace (e.g. directly calling `Prefab.RegisterExisting` from a non-mod-stack context). If such a mod exists, only the stack-trace hook or the incremental-dump fallback can attribute it. Confirm empirically against the actual mod set when the dump runs. (2026-06-21: ANSWERED -- yes, such a mod is present. Mirrored Devices registers its 18 `*Mirrored` prefabs by direct injection: it shows in `ModLoader.LoadedMods` with `Prefabs.Count == 0` and leaves no `Mod.AllMods` / `LoadedMod.Prefabs` trace, so registry attribution misclassifies all 18 as vanilla. Confirmed by an independent no-mods diff, 321 vs 378 power/data devices. The three `[StationeersMods]`-loader mods present, FilterCleanerMod / VeryImportantButtonMod / BowlingMod, DO trace via `ModLoader.LoadedMods`; Mirrored Devices is the lone untraceable case in this set, and it needs the vanilla diff or the `RegisterExisting` hook to attribute.)
