...
- For StationeersMods-style mods that do not use LaunchPadBooster, consult StationeersLaunchPad `ModLoader.LoadedMods` and each `LoadedMod.Prefabs` (`List<GameObject>`); each GameObject's `Thing.PrefabName` -> hash attributes to that `LoadedMod` (see StationeersLaunchPadModLoading.md). Map `LoadedMod` -> name via its `Def.About.Name` / `DirectoryName`.
