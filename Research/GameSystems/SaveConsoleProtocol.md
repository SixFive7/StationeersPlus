...
- 2026-08-15: the Open Question "whether stdin console commands reach `CommandLine.Process` on a batch-mode dedicated server" is **resolved: they do not, and there is no configuration in which they do.** Determined at 0.2.6428.27798 by a fresh validator under `Research/WORKFLOW.md` Rule 3. The server's console reader is `UI.ImGuiUi.RocketSystemConsole.ConsoleInputThread`, which calls `System.Console.ReadKey()` against the Win32 console input buffer; it is wired to the dispatcher by `_systemConsoleInput.OnInputReceived += CommandLine.Process;` and never reads a stdin stream. Writing a line into the process's redirected stdin therefore has no consumer, measured both with `-logFile` present (the console object is not even constructed) and with `-logFile` removed (console fully up, `Ready` printed, input thread started, and the write still did nothing). The chain documented on this page is unaffected: the transport was the only thing in question, and `ConsoleWindow.Submit` remains the in-process entry point that drives a save. Full evidence: [DedicatedServerSettings, "The console channel"](./DedicatedServerSettings.md).
...
(The stdin transport question that sat here is resolved; see the 2026-08-15 Verification History entry above.)
