...
Normalising every straight `Cable` to one canonical roll per run axis (the previous section) makes straight-to-straight seams flush, but **leaves the band misaligned at most straight-to-corner seams, and no choice of per-axis canonical roll can fix that** — it is a structural limitation, not a tuning problem. The reasoning, with the supporting evidence drawn from a 12k-thing save (`Luna.save`, game version 0.2.6228.27061, ~317 `StructureCableSuperHeavyCorner`, ~1600 `StructureCableSuperHeavyStraight`):

- A *straight* heavy/super-heavy cable's coloured band sits on the world face `WorldRotation * Vector3.up` points to (its local +Y face), and the four connectivity-equivalent rolls about its run axis put the band on the four world faces perpendicular to that axis.
- A *corner* heavy/super-heavy cable (`Cable.IsStraight == false`, e.g. `StructureCableSuperHeavyCorner`) has its band running through the L, exiting on the corner mesh's **local +Y face at both open ends** (the artist's mesh; derived empirically — for `StructureCableSuperHeavyCorner` the two open ends sit on local **-X** and local **-Z**, i.e. the corner connects whichever world directions `rot * (-X)` and `rot * (-Z)` map to). So the world face the corner's band exits to at a given end is `corner.RegisteredWorldRotation * Vector3.up` — a single value, the *same* at both of that corner's ends, fully determined by the corner's `WorldRotation`.
- For the band to flow continuously through a corner, the straight on each leg must have its band on `corner_rot * Vector3.up`. But the corner's `WorldRotation` is **not normalised by anything** — corner/tee/cross pieces have their open ends off the run axis, so re-rolling them changes connectivity; they are the smart-rotated-at-placement orientation and stay that way. Worse, the corner's `WorldRotation` is **not even a function of its leg-direction pair**: a corner connecting (say) a +X leg and a +Z leg can be the L "concave up" or "concave down" (the mesh flipped over), and the two variants expose the band on opposite world faces (in `Luna.save`, of 17 `(+X,+Z)`-leg super-heavy corners, 15 want band on world `-Y` and 2 want band on world `+Y`; the (+X,-Z) group is 9 want `+Y` vs 1 want `-Y`; etc.).
- Therefore the demanded band face, *for straights running along one fixed world axis*, is spread across multiple world faces. In `Luna.save`: X-running straights that flank a super-heavy corner are demanded band-on-`+Y` 67 times, band-on-`-Y` 59 times, band-on-`+Z` 14, band-on-`-Z` 14 (154 corner-legs total). Z-running: `+Y` 38, `-X` 30, `-Y` 30, `+X` 24. Y-running: `-X` 24, `+Z` 21, `-Z` 21, `+X` 9. No single roll satisfies more than ~43% / ~32% / ~31% of the corner-legs on the X / Y / Z axis respectively. And a single straight that sits *between two corners* can be demanded **contradictory faces on its two ends** (≈14 X-running, ≈14 Y-running, ≈8 Z-running such straights in `Luna.save`) — for those, no roll satisfies both ends, so the mismatch is intrinsic and would persist even if every corner in the world were placed at a canonical orientation.
- Net: over all 4×4×4 one-roll-per-axis assignments, the best on `Luna.save` mates ~36% of super-heavy corners on **both** legs (≈44% on at least one leg). The per-axis-plurality assignment is: X run → `Quaternion.Euler(0,90,0)` (band on world +Y, 43% of X corner-legs), Y run → `Quaternion.Euler(270,0,90)` (band on world -X, 32%), Z run → `Quaternion.identity` (band on world +Y, 31%). (`ZoopMod`'s Z-cable roll `Euler(0,0,90)` puts the band on world -X — the Z-axis *second* choice, 24%.) Changing the canonical set re-rolls every straight cable on the next world load — a one-time cosmetic conversion, harmless per "Changing a placed Structure's rotation at runtime" below.

Actually fixing every straight-to-corner seam requires abandoning "one roll per run axis" for corner-adjacent straights and instead re-rolling each such straight to match the *specific* corner it touches (read the corner's `WorldRotation`, put the straight's band on `corner_rot * up`), with a tie-break policy when a straight is between two disagreeing corners (and even then a long straight run between two corners can only match at one end — the straight mesh's band runs the length of the piece and cannot bend mid-run), and probably normalising the corners themselves to a canonical per-leg-pair orientation first (which is *not* a cosmetic-only change — re-rolling a corner is connectivity-relevant and needs the `SmartRotate` machinery or a careful hand-rolled equivalent). That is a much larger change than the straight-only roll sweep.
...
<!-- verified: 0.2.6228.27061 @ 2026-05-12 -->
<!-- updated: 0.2.6228.27061 @ 2026-05-21 — added "Empirical patch-attachment results" subsection: the SpawnConstruct prefix and Thing.Create<T> prefix recommendations below do NOT hold at runtime (inlining / generic-unpatchable) -->
...
### Empirical patch-attachment results (NetworkPuristPlus runtime, 0.2.6228.27061)
<!-- verified: 0.2.6228.27061 @ 2026-05-21 -->

The candidate hooks above were runtime-tested in NetworkPuristPlus v1.1.x for canonicalising a freshly built straight cable's roll. Three recommendations above do NOT hold in practice; the on-load sweep is the only robust mechanism.

- **A `Thing.Create<T>` prefix cannot be applied.** HarmonyX throws `HarmonyLib.HarmonyException: IL Compile Error (unknown location) ---> System.NotSupportedException: Specified method is not supported` at `Harmony.PatchAll()` when the target is the open generic method definition `Thing.Create<T>(Thing, Vector3, Quaternion, long)` (via `AccessTools.Method(...)` from a `TargetMethod`). The exception aborts the whole plugin's `PatchAll`, so the mod fails to initialise. Open generic method definitions are not patchable here, so the "single chokepoint" recommendation above is not achievable as written.
- **A `Constructor.SpawnConstruct` prefix does not attach.** `SpawnConstruct` is a tiny method (`Thing.Create<Structure>(...)` + `structure.SetStructureData(...)` + `return structure`), so Mono inlines it; a Harmony prefix on it silently never runs, with no error at `PatchAll`. Verified: an unconditional prefix logged **0 entries across 76 confirmed `SpawnConstruct` calls** during an on-load long-piece rebuild. This invalidates the "a Harmony prefix on `Constructor.SpawnConstruct(CreateStructureInstance instance)` mutating `instance.LocalRotation` is clean" recommendation above, and means any pre-existing `SpawnConstruct` patch that was never exercised in-game is also dead.
- **A `SetStructureData` prefix is insufficient for the visible roll.** `Structure.SetStructureData(Quaternion localRotation, ulong, Grid3, int)` (`:297340`) sets only `Direction = localRotation` (a network-sync-only field, see the next section); it does not touch the transform or `RegisteredRotation`. The visible/saved rotation comes solely from `Thing.Create<Structure>(instance.Prefab, instance.WorldPosition, instance.WorldRotation, 0L)` inside `SpawnConstruct`. Mutating `SetStructureData`'s `localRotation` therefore fixes only `Direction`, not the mesh roll.
- **A `Cable.OnRegistered` postfix fires but does not replicate to a multiplayer client.** The postfix runs host-side and re-rolls the cable's transform + `RegisteredRotation` + `Direction` + sets `NetworkUpdateFlags |= 1`, but a freshly built cable on a remote CLIENT keeps its pre-roll rotation: the new-thing replication captured the rotation before the postfix ran, and the per-tick transform-delta did not correct the just-created client copy. (`SetStructureData`, which runs after `OnRegistered` inside `SpawnConstruct`, also resets `Direction` back to the pre-roll value.) It works host / single-player only.

Net: instant build-time canonicalisation of a placed `Structure`'s rotation is not cleanly achievable through these chokepoints, because the only universal funnel (`Thing.Create<T>`) is generic-unpatchable and its non-generic caller (`SpawnConstruct`) is inlined. The robust mechanism is the **on-load sweep** (re-roll every placed straight cable at `World.OnLoadingFinished`), which catches a client-built cable on the next world load. A host/single-player-only instant re-roll via the `Cable.OnRegistered` postfix is additive on top of that. True remote-client instant align would require patching the non-inlined network message handlers per build path (`CreateStructureMessage.Process` for multi-kit / the cable coil, `ConstructionCreationMessage.Process` for a single-Constructor build).
...
- 2026-05-12: added "Why straight-cable roll normalisation does not fix the band-seam at a corner cable" after analysing a 12k-thing save (`Luna.save`, game version 0.2.6228.27061) with NetworkPuristPlus's straight-roll normalisation already applied — a reported visual glitch where a normalised straight super-heavy cable mates with an adjacent `StructureCableSuperHeavyCorner` on one leg but not the other. Sources: parse of `world.xml` (`AllThings`, the `CableSaveSaveData` entries — `RegisteredWorldPosition` on the 0.5 m small grid, `RegisteredWorldRotation` quaternion, `CableNetworkId` — for ~317 `StructureCableSuperHeavyCorner` and their flanking `StructureCableSuperHeavyStraight` pieces); cross-checked against the existing "Straight pipe/cable/chute roll" and "Changing a placed Structure's rotation at runtime" sections plus the `KeyMap.Rotate*` cursor-key assignments (`Assembly-CSharp.decompiled.cs` 0.2.6228.27061 `:43346-43351`). Findings: the heavy/super-heavy *corner* cable mesh carries its band on local +Y at *both* open ends (derived empirically — `StructureCableSuperHeavyCorner` open ends sit on local -X and local -Z); the world face the corner's band exits to is `corner.RegisteredWorldRotation * Vector3.up`, which is *not* normalised (corners are not roll-canonicalised — re-rolling them is connectivity-relevant) and is *not* a function of the corner's leg-direction pair (the "L flipped over" cases expose the band on opposite world faces); consequently the demanded band face for straights along one world axis is spread across multiple world faces (≈43/38/9/9 split for X-running straights in `Luna.save`), a single straight between two corners can be demanded contradictory faces on its two ends (≈14 X / ≈14 Y / ≈8 Z such straights), and no one-roll-per-axis canonical assignment exceeds ~36% of super-heavy corners mated on both legs; per-axis-plurality assignment is X→`Euler(0,90,0)` (band +Y), Y→`Euler(270,0,90)` (band -X), Z→`Quaternion.identity` (band +Y). Additive; no conflict (the existing "Straight pipe/cable/chute roll" section already states corners must not be re-rolled and that straight roll is connectivity-blind; this section explains why straight-only normalisation leaves corner seams misaligned and why no per-axis choice fixes it). Originating mod-side write-up: NetworkPuristPlus `RESEARCH.md` and `.work/2026-05-12-v1.1-polish/corner-investigation.md`.

- 2026-05-21: added "Empirical patch-attachment results" subsection to "Build-time structure-creation chokepoints" after NetworkPuristPlus v1.1.x runtime-tested the candidate hooks from the 2026-05-12 chokepoint pass. Corrects recommendations from that pass that did not hold in practice: HarmonyX cannot patch the open generic `Thing.Create<T>` (`Specified method is not supported` at `PatchAll`, aborting init); `Constructor.SpawnConstruct` is inlined so a prefix on it silently never attaches (0 prefix entries across 76 confirmed calls during an on-load rebuild, no error); `Structure.SetStructureData` sets only `Direction`, not the transform/`RegisteredRotation`; and a `Cable.OnRegistered` postfix's host-side re-roll does not reach a freshly built cable on a multiplayer client. Source: NetworkPuristPlus dedicated-server runtime (BepInEx `LogOutput.log` / dedicated `server.log`, 0.2.6228.27061) plus re-reads of `Constructor.SpawnConstruct` (`:276940`) and `Structure.SetStructureData` (`:297340`). The on-load sweep remains the robust mechanism; no section other than the chokepoints subsection is affected.
...
- Whether the heavy/super-heavy *corner* cable mesh genuinely carries its band on local +Y at both open ends, or whether a different local face applies at one end. Derived empirically here from one matching straight-corner pair in `Luna.save` plus the observation that the +Y-at-both-ends hypothesis is the only single-local-face assumption that yields a self-consistent per-axis answer for any corners; not confirmed against the prefab mesh. Resolve by inspecting the `StructureCableSuperHeavyCorner` / `StructureCableHeavyCorner` mesh in the asset bundle, or via an InspectorPlus snapshot correlating several corners' `RegisteredWorldRotation` with their visually-confirmed band orientation.
