...
## Lookup by id: the global-namespace Referencable registry
<!-- verified: 0.2.6403.27689 @ 2026-07-18 -->

`CableNetwork` instances (and every other `IReferencable`) resolve by ReferenceId through `Referencable.Find<T>(long)`, a static class declared in the GLOBAL namespace (decompile line 37867), not under `Assets.Scripts.Objects`. The lookup is a plain managed dictionary read, safe off the main thread (the pattern PowerGridPlus uses from its tick worker). Vanilla itself resolves merge/split survivors this way (for example decompile 272141, 272420).

## Verification history

- 2026-07-18: added the Referencable global-namespace lookup note (decompile 37867; vanilla call sites 272141/272420).
