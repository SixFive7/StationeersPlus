...
It fires `OnDeviceConnectToNetwork(device)` on every other device on the network: the cascade a real device-add rides. It is SEPARATE from the list rebuild. `CableNetwork.RefreshPowerAndDataDeviceLists` (`:253589`) and `HandleDataNetTransmissionDevice` (`:253630`) mutate the cached `_dataDeviceList` silently and fire no per-device callback (see [CableNetwork.md](./CableNetwork.md)). `AddDevice` (`:253852`) / `RemoveDevice` (`:253873`) additionally raise the static `CableNetwork.OnNetworkChanged` action (a coarse "some network changed" signal, distinct from the per-device cascade). `RemoveDevice` is the symmetric teardown: it calls `HandleDeviceDisconnection` (`:253895`), which iterates the remaining `DeviceList` and fires `OnDeviceDisconnectFromNetwork(device)` on each (mirroring `RefreshNetworkDevice` for adds), then dirties. So a device LEAVING a network does cascade its consumers via vanilla, which is why deconstructing a bridge device (transformer / battery / dish) refreshes the affected motherboards without a mod trigger. Only a change with NO physical add/remove, a same-membership passthrough merge or a wireless dish link / unlink, skips the cascade and needs an explicit refresh.
...
- 2026-05-28: extended the cascade section with the removal-side path: `CableNetwork.RemoveDevice` (`:253873`) calls `HandleDeviceDisconnection` (`:253895`), which fires `OnDeviceDisconnectFromNetwork` on every remaining member (symmetric to `RefreshNetworkDevice`). Confirms a bridge deconstruction refreshes consumers via vanilla, while a same-membership passthrough merge or a wireless dish link/unlink (no physical add/remove) does not. Sourced during the PowerGridPlus completeness audit.
