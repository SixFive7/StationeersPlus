...
- Does the tooltip name box background grow vertically to fit a multi-line `Title`? Not decidable from code (no code-side sizing; see the Title-row note under the SetUpToolTip subsection): the answer lives in the serialized tooltip prefab (RectTransform dimensions, layout components, TextMeshPro overflow mode). Pending the PowerGridPlus merged-layout playtest, which puts a three-line Title on a faulted transformer's button hover (`Mods/PowerGridPlus/PLAYTEST.md`).
