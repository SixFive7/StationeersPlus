...
- `WirelessPower : ElectricalInputOutput, IRotatable` (decompile line 405441). Both `PowerTransmitter` and `PowerReceiver` inherit two `CableNetwork`-typed slots from `ElectricalInputOutput`: `InputNetwork` and `OutputNetwork`. Directional usage in vanilla:
