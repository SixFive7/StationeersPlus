...
## Enrollment corollary: never OnOff-gate the Female in a replacement allocator
<!-- verified: 0.2.6403.27689 @ 2026-07-14 -->

The Female quartet's OnOff-blindness (gate table above) is load-bearing for any mod that replaces the vanilla power tick: a generic "skip switched-off segmenters" enrollment filter silently drops the Female, and since she is typically a docked rocket's ONLY grid-side supply, the rocket's internal cable network then models zero supply, gets judged dead, and every rocket-mounted battery's charge grant is zeroed. Observed live 2026-07-14 in PowerGridPlus: `StructureBatteryMedium` 703648 sat at 0 J next to Female 614953 holding a full 4900 J cell (her `OnOff` is false in normal play; the pair's player-facing switch is the Male). The fix exempts `RocketPowerUmbilicalFemale` from the OnOff enrollment gate, matching her vanilla semantics; the Male keeps the gate (vanilla gates his quartet on OnOff).

## Verification history
- 2026-07-14 (third pass, same day): added "Enrollment corollary" (the OnOff-blind Female must not be dropped by a generic switched-off-segmenter filter; PowerGridPlus rocket-battery starvation found and fixed live). Additive; grounded in the gate table already on this page.
