...
- **Superseded by the base game in 0.2.6332.27293.** <!-- verified: 0.2.6403.27689 @ 2026-07-27 --> The worked example above is historical. The shipped per-build changelog (`rocketstation_Data/StreamingAssets/version.ini`, under `[Version 0.2.6332.27293]`) reads "Fixed Structure Frame Corner and Side could not be painted. Now they can." All four Steel Frame shapes now ship with `PaintableMaterial` set, so the copy-from-donor workaround is a no-op on 0.2.6403.27689 and later. Confirmed on a live client running only SprayPaintPlus, StationeersLaunchPad and ClientDriver: SprayPaintPlus's donor-copy pass resolved both prefabs, found `PaintableMaterial` already non-null on each, and reported `Extra paintability: 0 structure(s) made paintable.` The technique itself is unchanged and still applies to any other prefab shipping a null `PaintableMaterial` slot.
...
- 2026-07-27: corrected the steel-frame worked example in "CustomColorMapping build rule (which renderers recolor)". The 2026-06-20 entries above recorded `StructureFrameCorner` and `StructureFrameSide` as shipping with a null `PaintableMaterial`, which was true on 0.2.6228.27061 but is no longer true: the base game fixed it in build 0.2.6332.27293 ("Fixed Structure Frame Corner and Side could not be painted. Now they can.", quoted verbatim from `rocketstation_Data/StreamingAssets/version.ini`, which ships the per-build changelog in the install). 0.2.6332.27293 was an internal build; the first public release carrying the fix is 0.2.6403.27689 (the Sanitation Update, 2026-07-02), because no stable release shipped between 2026-03-26 and 2026-07-02. Confirmed empirically the same day on a live client with exactly three plugins loaded (SprayPaintPlus 1.11.0, StationeersLaunchPad 0.5.0, ClientDriver): SprayPaintPlus's donor-copy pass ran with its setting enabled, resolved both prefabs with no "not found" warning, found `PaintableMaterial` already non-null on both, and logged `Extra paintability: 0 structure(s) made paintable.` Additive: the historical example is kept with its 0.2.6228.27061 label and a superseding bullet was added beneath it. No fresh validator required, since the game version advanced rather than the earlier reading being wrong. The section stamp is deliberately left at 0.2.6228.27061 because only this one worked example was re-checked against 0.2.6403.27689, not the rest of the section; the new bullet carries its own inline stamp. Bumped frontmatter verified_at.
