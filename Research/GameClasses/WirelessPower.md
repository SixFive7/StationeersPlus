...
- 2026-07-07: re-confirmed `WirelessNetwork : CableNetwork` (272453) and its membership shape (the carrier network's power members are exactly the two dish halves, both registered segmenters) against the 0.2.6403.27689 decompile. Occasion: the PowerGridPlus partial-power sentinel's carrier-network exclusion (bridge-only membership means a sub-1 vanilla ratio on a carrier deprives no device; under the billing handshake the carrier's demand mirror is the receiver's unclamped drain while its supply mirror is the delivery-gate-clamped advertise, so ratio below 1 is its normal conducting state). No content change.
