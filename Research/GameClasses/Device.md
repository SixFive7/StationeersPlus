...
**`_powerProvided` (one-tick lag, on the pass-through `ElectricalInputOutput` devices: Transformer, AreaPowerControl, RocketPowerUmbilical).** This is the accumulator of the power drawn THROUGH the device by its downstream (output-network) consumers. It is added to in the device's `UsePower` and decremented in `ReceivePower` as input power flows in (see [PowerTick](./PowerTick.md): `ApplyState` -> `ConsumePower` -> `Device.ReceivePower`). The pass-through device's input-side draw is reported FROM this accumulator: vanilla `Transformer.GetUsedPower(InputNetwork)` returns `min(Setting + UsedPower, _powerProvided)`, and `AreaPowerControl.GetUsedPower(InputNetwork)` returns `UsedPower + _powerProvided` (+ a cell-charge term). Because `_powerProvided` is filled during `ApplyState` (which runs AFTER `CalculateState` has already summed `GetUsedPower` for the tick), the value a `CalculateState` reads is last tick's downstream consumption: **the input-side draw a pass-through device bills lags its output-side delivery by exactly one tick.** This is the mechanism behind the "transformer free-power / one-tick-lag" exploit family and is what Power Grid Plus replaces with a fresh allocator-computed throughput.
