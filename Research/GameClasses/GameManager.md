...
- 2026-07-18: partial re-check against the 0.2.6403.27689 decompile while validating PowerGridPlus join-time burn-reason ordering. `UpdateThingsOnGameStart` still dispatches per-Thing `OnFinishedLoad` through the same delegate (decompile 203847; `thing.OnFinishedLoad()` at 203855), with one evolution: the delegate body is now wrapped in a try/catch that prints `OnFinishedLoad failed for {DisplayName} #{ReferenceId}: {exception}` per failing Thing, so one throwing `OnFinishedLoad` no longer aborts the rest of the pass (the section excerpt above shows the 0.2.6228 shape without the try/catch). Also confirmed the pure-client join path calls `GameManager.UpdateThingsOnGameStart()` directly from `NetworkClient.ProcessJoinData` (decompile 213188+) after the join reader closes; see `Research/Protocols/PlayerConnectedThingFindTiming.md` for the full join ordering. `StartGame` itself was not re-read this pass, so section stamps are unchanged.
