...
Adjacent but a *separate* system, not a `MultiMergeConstructor`: the conveyor-belt cargo system has `StructureConveyorStraight` / `StructureConveyorStraightLong` / `StructureConveyorStraightShort` / `StructureConveyorCorner` / `StructureConveyorRiser` in kit `ItemKitConveyor` ("Kit (Conveyors)"). `StructureConveyorStraightLong` is a distinct multi-cell buildable variant you select from the (plain `MultiConstructor`) kit; there is no merge-absorb of short conveyors into a long one. Also unrelated: `StructureCompositeCladdingAngled*Long*` and `StructureLightLong*` are cladding panels / light fixtures with a wider collider, not multi-cell pipe-style runs.
...
(On the *prefab*, use `GridBounds.GetLocalSmallGrid`, not `Structure.BlockingGrids` — `BlockingGrids` is the post-placement registered set, a `Grid3[1]` placeholder on a fresh prefab. `Structure.ForceGridBounds` overrides flow through `GetLocalSmallGridBounds`, so the cell-count check still holds. Optionally corroborate with `(s as Cable)?.BlockMergeWithOtherCables == true || (s as Piping)?.DontAllowMergingWithWrench == true`.)
...
3. **Postfix `World.OnLoadingFinished`** (`Assembly-CSharp` line ~305850; or `GameManager.StartGame` ~189593, which it calls): `if (!GameManager.RunSimulation) return;` then iterate a snapshot — `foreach (Structure s in GridController.AllStructuresPool.ToList()) if (s != null && !s.IsBeingDestroyed && longHashes.Contains(s.PrefabHash)) OnServer.Destroy(s);`. By this point `XmlSaveLoad.LoadWorld` -> `LoadThing` (which needs `Prefab.Find` to still resolve the long-variant prefabs) has run, networks are linked, `Thing.OnFinishedLoad` has fired per Thing, and `GameState == Running`. `OnServer.Destroy` deregisters the cell from `GridController`, splits the pipe/cable/chute run there (surrounding 1-cell pieces become disconnected ends, no NRE), and queues a `DestroyEvent` to clients; it's server-only (the `RunSimulation` guard covers that). This is the same iterate-and-`OnServer.Destroy` pattern as `WorldManager.DeleteOutOfBoundsObjects` (~251634). Plain delete loses the long segment's atmosphere/contents and splits the run; the "replace with N x 1-cell base" alternative (lever 5, NetworkUpgrader-in-reverse) preserves both but is more work.
4. **Do not** add a hard reject in `MultiMergeConstructor.Construct` / `Constructor.SpawnConstruct` (lever 3) — step 2 already makes the wheel unable to select a long variant, and a mid-placement reject breaks ZoopMod.

`GridController.AllStructuresPool` (`Assembly-CSharp` line ~191276, `DensePool<Structure>("AllStructuresPool", 65536)`) is the canonical "all placed structures" collection (every `Structure` / `SmallGridObject` is added in `GridController.Register`, removed in `Deregister`); `OcclusionManager.AllThings` is the broader all-Thing pool if you need it.
