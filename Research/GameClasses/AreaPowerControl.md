...
## Visual state machinery: the charge LED, the lever, and their write cadence
<!-- verified: 0.2.6403.27689 @ 2026-07-18 -->

Three serialized visual components on the class (390564-390570): `_chargingLedMaterialChanger : ApcMaterialChanger` (the power / charge LED), `_onOffAnimComponent : ApcOnOffAnimComponent` (the lever), and `_openAnimComponent` (the door).

The lever is a TRANSFORM animation only: `ApcOnOffAnimComponent` (33148-33162) extends `AssignableBinaryAnimComponent : BinaryTransformAnimComponent` (33188-33259), keyframing transforms plus click sounds. Vanilla never writes lever materials, so a mod pulsing the lever's material is uncontested.

The LED is driven by `ApcMaterialChanger : MaterialChanger` (191917-192064; base 192195-192225 holds one serialized `MeshRenderer` plus `MaterialAnimState[]`, applied via `MaterialAnimState.ApplyTo`, 192238-192249; state ids are `Defines.Animator` hashes, 228195-228263; the actual colors live in prefab material assets, not code). Namespace + field declarations for patch authors: `ApcMaterialChanger` and its base `MaterialChanger` both live in the bare `Effects` namespace (namespace block 191915-192302), not `Assets.Scripts.*` and not the global namespace; the base's serialized renderer field is `protected MeshRenderer renderer` (192200-192201), and the owner reference is the serialized `public Thing parent` field on `ApcMaterialChanger` itself (191919-191920), wired to the `AreaPowerControl` by the prefab. Write trigger: `Thing.OnInteractableUpdated` (319517-319522) calls `RefreshAnimState`, and the APC override (390689-390695) forwards to `_chargingLedMaterialChanger.RefreshState()`. `OnPowerTick` (390952-390981) recomputes `PowerMode` each tick but only fires an interact when the mode CHANGES.

`RefreshState()` (191966-192013) dispatches per state, and three of the states start FREE-RUNNING async blink loops that rewrite `renderer.materials` for as long as the state predicate holds (they poll `parent.Mode/Error/OnOff`, not RefreshState calls):

- Error (Error==1 and OnOff and Powered): `ErrorAnim` alternates `Error0`/`Error1` every 250 ms with an error beep (192015-192029).
- Charging (Mode==3): `ChargingAnim` alternates `Charging`/`Charged` every 500 ms (`APC_FLASH_DELAY = 500`, 191928) with the ApcCharging sound per cycle (192031-192046). This 1 Hz blink is the LED flicker a player sees while the cell charges.
- Discharging (Mode==2): `DischargingAnim` alternates `Charging`/`Discharged` every 500 ms (192048-192063).
- Idle / Charged / Discharged / Off: one single material write on the state change (Charged and Discharged also play a one-shot sound).

Cadence summary: event-driven single writes on mode or interactable changes, plus 250-500 ms free-running rewrites during Error / Charging / Discharging. A mod that wants to own the LED renderer during those states must either re-assert its material after the loop writes (a LateUpdate re-assert lands last in the frame and wins visually) or suppress the loops at their state predicates.

## Verification history

- 2026-07-18: appended the namespace + field-declaration detail to the visual-state-machinery section (`Effects.ApcMaterialChanger` / `Effects.MaterialChanger`, namespace block 191915-192302; base serialized `protected MeshRenderer renderer` 192200-192201; serialized `public Thing parent` 191919-191920), confirmed against the 0.2.6403.27689 decompile via a whole-file `^namespace` census while wiring the PowerGridPlus APC charge-LED flash retarget. Additive; no existing claim changed.
- 2026-07-18: added the visual-state-machinery section (charge LED ApcMaterialChanger blink loops at 250/500 ms, transform-only lever, OnInteractableUpdated write cadence), read from the 0.2.6403.27689 decompile during the fault-flash placement research; adversarially cross-read this session.<!-- verified: 0.2.6403.27689 @ 2026-07-06 -->
