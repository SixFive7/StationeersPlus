...
**The matching trap in the other direction:** `Debug.LogError` and `Debug.LogException` DO appear in the in-game console. `ConsoleWindow` subscribes to `Application.logMessageReceivedThreaded` (`Assembly-CSharp.decompiled.cs:221927`) and its handler (`:222266-222282`) re-prints `LogType.Error` and `LogType.Exception` itself, lowercased, prefixed `[ERROR]`, in red, followed by the stack trace. So calling `Debug.LogError` **and** a `ConsoleWindow` method for the same message shows it to the player twice. Pick one.
