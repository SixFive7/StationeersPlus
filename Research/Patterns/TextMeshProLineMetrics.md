...
- `font_english`'s `normalSpacingOffset` is assumed 0 (the TextMeshPro default). It shifts the cspace cancel by `normalSpacingOffset * 0.06` per character in the tooltip case. Verify with an InspectorPlus probe on a graphical client (`types=[TMP_FontAsset]`, `fields=[normalSpacingOffset]`); the headless dedicated server cannot resolve TMP_FontAsset instances.
