...
sources:
  ...
  - .work/decomp/0.2.6428.27798/Assembly-CSharp.DedicatedServer.decompiled.cs:107246-107460 (UI.ImGuiUi.RocketSystemConsole, ConsoleInputThread at :107330), :215144-215231 and :215503-215511 (Assets.Scripts.ConsoleWindow console wiring)
...
- **Platform support:** Windows and Linux server builds. The constructor throws `"Don't use this outside of dedicated server builds"` unless `Application.platform` is `LinuxServer` or `WindowsServer`.
- **Features:**
  - UTF-8 encoding
  - Input thread for console commands
  - Title bar with game version and command-line args
  - Input forwarded **directly** to `CommandLine.Process`, by `_systemConsoleInput.OnInputReceived += CommandLine.Process;`. It does NOT go through `ConsoleWindow.Submit`, which this page claimed until 2026-08-15; `Submit` is the in-game console's own path and prints an echo the console channel does not.
...
**The input side is keystrokes, not a stream, and it cannot be driven by writing to the process's stdin.** Verified at 0.2.6428.27798 against the DEDICATED SERVER build (`.work/decomp/0.2.6428.27798/Assembly-CSharp.DedicatedServer.decompiled.cs`; line numbers in this paragraph are from that file, not from the client decompile the rest of this section cites):

- `RocketSystemConsole.ConsoleInputThread` (:107330) loops on `System.Console.CursorVisible` and `System.Console.KeyAvailable` and calls `System.Console.ReadKey()` (:107335, :107339). Those are the Win32 console input buffer, reached through `PeekConsoleInput` / `ReadConsoleInput`. An anonymous pipe on the child's stdin is a different object and is never read. `ReadKey` and `KeyAvailable` each occur exactly once in the whole assembly, both here.
- `Enter` triggers `OnEnter()`, which strips the `"> "` prefix, trims, and enqueues under `lock (_inputQueue)`. A separate `Tick()` loop drains that queue **only when running on the Unity thread**, then raises `OnInputReceived`. That marshalling is why console commands are safe against game state.
- `_inputThread.Start()` is called from exactly one place, `Ready()` (:107307), which is itself called only from `ConsoleWindow.WaitForGameToBeReadyThenOverrideConsoleInput()` (:215503) after `GameManager.IsInitialized` plus a 1 s delay, and under the same `!CustomLogFile` guard. With `-logFile` on the launch line there is no console object, no input thread and no handler.
- Measured 2026-08-15 on 0.2.6428.27798, `-logFile` REMOVED so the console was fully up (stdout carried `***Stationeers - 0.2.6428.27798***` then `Ready`): `settings ServerMaxPlayers 9` written to the process's redirected stdin and flushed changed nothing, and printed nothing. Full write-up and the paired control arms: [DedicatedServerSettings, "The console channel"](../GameSystems/DedicatedServerSettings.md).
...
- 2026-08-15: **conflict on "how does the dedicated server's console input reach the command dispatcher".** Previous claim (detail item 8, stamped 0.2.6403.27689): "Input forwarded to CommandLine.Process via ConsoleWindow.Submit (Assembly-CSharp:222487)". New finding, from a fresh validator's own decompile of the DEDICATED SERVER `Assembly-CSharp.dll` at 0.2.6428.27798: the console subscribes directly, `_systemConsoleInput.OnInputReceived += CommandLine.Process;` (:215509), and never touches `Submit`. Verdict: **the new finding is correct**; the token `CommandLine.Process` occurs exactly four times in the assembly and :215509 is one of them, while `ConsoleWindow.Submit`'s own call to `CommandLine.Process` (:215637) is a separate route used by the in-game console. Practically the distinction matters because `Submit` prints a Cyan echo of the command and the console channel does not, so a detector keyed on that echo would miss real console input. Result: detail item 8 corrected and restamped, and expanded with the input-side mechanism, which had never been recorded: the thread reads `System.Console.ReadKey()` against the Win32 console input buffer rather than any stdin stream, `Enter` marshals the assembled line onto the Unity thread through a locked queue, and `_inputThread.Start()` is reached only through `Ready()` under the same `!CustomLogFile` guard that gates construction. This page had already recorded the `-logFile` gate correctly since 2026-07-27 (items 8 and 9), and that half is upheld unchanged; it was `GameSystems/DedicatedServerSettings.md` that lacked it. See that page's 2026-08-15 entry for the paired live arms.
