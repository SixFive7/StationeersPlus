...
The client's `<install>/BepInEx/config/` contains `.cfg` files for every BepInEx-style mod the client has loaded (e.g. `BetterAdvancedTablet.cfg`, `JetpackHeightUnlocker.cfg`, dozens more). Mirroring this directory to the server is documented in `TestRig/MANUAL.md` as part of `update-game -Target server`.
