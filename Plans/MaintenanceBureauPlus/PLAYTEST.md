...
- **Console mirror line prefix changed (uncommitted, 2026-07-27).** `LaunchPadLog.ForwardToStationeersConsole` prefixed every mirrored line with `"[MBP] "`, which is a named violation of the no-abbreviations rule in this mod's own `CLAUDE.md`. It now emits `"[MaintenanceBureauPlus] "`. Only the prefix string changed; the severity-to-colour mapping and the `aged: true` choice are untouched, and the rest of that pass was comment corrections (the file header now states the flooding rate, the cross-thread hazard, and the `aged` semantics accurately).
