...
Side effect: the bureau is not listed in LaunchPad's in-game mod list, and StationeersLaunchPad's own log viewer (`slp logs`) does not show its messages, because a mod LaunchPad did not load has no LaunchPad child `Logger`. The game's own in-game console (`KeyMap.ToggleConsole`, F3 by default) is a different surface and DOES show them today, through the reflection bridge in `LaunchPadLog.cs` that is slated for removal before v1.0.0 (see "Playtest gate" below). All logs go to `<STATIONEERS_INSTALL>\BepInEx\LogOutput.log` and the Unity player log at `%USERPROFILE%\AppData\LocalLow\Rocketwerkz\rocketstation\Player.log`.
...
- **Verify the multiplayer-compatible and safe-to-remove claims** added to `README.md` and `About/About.xml` (`<Description>` + `<InGameDescription>`): "Full multiplayer compatibility. Safe to remove from existing savegames." Both claims are unverified for this mod specifically. The bureau modifies save state in two ways the other Plus mods do not: the repair sweep zeroes incident damage on most things in the world, and the approval event spawns a `LanderCapsule` per player. Confirm before publish: (a) a client without the mod connecting to a server running the bureau joins cleanly and sees bureau chat in the public channel, with no version-handshake error or missing-mod warning; (b) a save written by a server that has run at least one full bureau cycle (chat trigger -> approval -> repair sweep -> capsule spawn) opens cleanly on a vanilla server with no XmlSerializer errors, no orphaned Thing references, and no broken capsule entities. If either claim fails, fix the underlying issue or strike the corresponding sentence from `README.md` and both `About.xml` slots before the next version ships.
...
- **Remove the playtest log bridges in `LaunchPadLog.cs`.** During playtest the file forwards every BepInEx log entry to (1) `StationeersLaunchPad.Logger` for the `slp logs` viewer and (2) `Assets.Scripts.ConsoleWindow.Print` for the in-game F3 console. Beyond the noise, the ConsoleWindow branch is a correctness bug and not just a nuisance: `OnLogEvent` runs on whichever thread called `Log.*`, and the model-load, inference, and watchdog threads all log through it, while `ConsoleWindow.Print` is main-thread-only (it shifts an unlocked 1024-entry static array while the draw loop reads it). The branch bypasses the mod's own `MainThreadQueue` entirely. It also forwards unescaped model output and verbatim player chat, which matters because a dedicated server launched without `-logFile` silently drops any line containing `<color=`. The ConsoleWindow branch in particular floods F3 with `[Watchdog #N]` and per-turn `[DIAG]` lines at a rate that makes typing other commands miserable for a Workshop subscriber. Either delete `LaunchPadLog.cs` entirely (and unwire `LaunchPadLog.Hook(Log)` and `LaunchPadLog.TryUpgradeToModLogger()` in `Plugin.cs`), or strip the ConsoleWindow branch and gate the LaunchPad branch on Warning+ only. Also drop the per-turn `[DIAG]` logging at every site in `ChatPatch.cs` and the `[Watchdog #N]` line cadence in `Plugin.StartWatchdog`.
