...
Every state renders ONE merged block, identical on the casing tooltip and the on/off button's title box. Line 1 is `{On|Off} - {title}: {n}s`: the switch word green `#00FF00` when on / grey `#9aa0a6` when off, the dash grey, the fault title and countdown red `#ff2626`; switchless devices (no `HasOnOffState`) drop the `{On|Off} - ` prefix; the two info states use their grey / amber titles and carry NO countdown. The diagnostics line(s) below are calm-grey `#9aa0a6` prose with the coloured values nested inside, one learnable rule across every block: fault red `#ff2626` = power wanted (downstream demand, cable flow, the shortfall / overage) and the offending device names; informational blue `#008AE6` = power available (upstream supply, device capacity, wire rating, throttle maximum); teal `#2DD4BF` = the internal-storage (battery) contribution split out from the available total; orange `#FF8000` = the IC10 `Setting` field, the exact colour Stationpedia paints a logic field. Exactly one block per hover: lower-precedence faults are not stacked (§11.5) and the info states are suppressed while any fault is active (the throttle note no longer stacks under a fault). The button's Action box keeps the pure vanilla action word.
