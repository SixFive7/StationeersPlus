...
The APC is the one `QuiescentAlwaysOn` contributor (§8.0.3 pass 1): vanilla bills its 10 W quiescent whenever `OnOff && OutputNetwork != null` regardless of throughput, so the allocator funds an idle APC's bare quiescent as a quiescent-only pull (`Pull = 10, Throughput = 0`) and the patch bills it under the same vanilla gate only while the published `TransformerSupplyCache` pull is positive: an idle healthy APC publishes `(0, quiescent)` and keeps billing it, an inactive (shed / overloaded / cycle-locked) APC publishes all-zero totals and bills 0 like every other inactive contributor, and a roster-absent APC (cache miss) bills 0, a deliberate vanilla deviation matching the errored-transformer 0-bill. The cell-charge portion bills 0 when the APC has no fresh `SoftDemandShareCache` share this tick (roster-absent: errored, short-circuited, or output-less), closing the unfunded phantom-draw hole the partial-power sentinel exposed (2026-07-07). Delivery is aligned with the bill (`DeliveryTickLedger`): the funded quiescent burns from the delivered stream at most once per tick (vanilla serves a bill in one ReceivePower call per contributing provider, so any per-call subtraction over-applies on multi-provider networks), the cell credit comes straight out of the stream cumulatively capped at the granted share and is recorded at the source for the charge-delivery audit, and only the passthrough remainder moves the vanilla `_powerProvided` ledger, keeping the cell-cover feature intact. A fresh share with a cell that can take no charge marks the grant moot for the audit (`MarkChargeGateClosed`, the Mode-based `IsCharged` fill edge).
...
- **Per-network rigid demand + generator supply.** For every device on a network that is NOT a segmenter (segmenters are modelled structurally), `GetUsedPower > 0` adds to `RigidDemand` and `GetGeneratedPower > 0` adds to `GenSupply`. VVF-faulted producers already read 0 here (their enforcement postfix), so a silenced producer contributes no supply. One exception rides this walk: a rocket umbilical half's own idle draw is funded here as plain rigid demand on its input network under the per-half vanilla gates (`RocketUmbilicalPatches.QuiescentBill`: Male whenever ON, Female whenever wired and not errored; the Buffered adapter has no routed seg to carry a quiescent pull), so the vanilla bill is never a phantom no advertise covers.
...
4. **Elastic shares** (§7.3 + §9.2) -> `SoftSupplyShareCache`: per output network, each battery / APC cell / umbilical's share is its RIGID component (the residual rigid shortfall `RigidDemand + PullsGranted - GenSupply - InflowCommitted`, floored at 0, full-or-proportional against effective caps) plus its SOFT TOP-UP (its full-or-proportional-to-leftover slice of the net's converged `ElasticFundedSoft` quantum), totalling at most `EffDischarge` per elastic. The tail also publishes the per-store discharge-grant snapshot (battery / umbilical only) for the discharge-delivery audit.
...
- `AreaPowerControlPatches.GetUsedPowerPatch` bills quiescent (vanilla gate `OnOff && OutputNetwork != null`, AND a positive published `TransformerSupplyCache` pull: inactive or roster-absent APCs bill 0 quiescent) + the `ApcPassthroughCache` passthrough (input == output, no lag) + the cell-charge portion capped to its `SoftDemandShareCache` share; a missing share (roster-absent APC) bills 0 charge. Its `ReceivePowerPatch` aligns delivery with that bill: funded-quiescent burn at most once per tick, cell credit capped at the granted share, passthrough remainder to the ledger (§7.5).
- `RocketUmbilicalPatches` bills each half's vanilla quiescent (funded as plain rigid demand by GATHER under the per-half gates, §8.0.3) plus the granted charge share; a missing share bills 0 instead of vanilla's full-headroom fallback. Its `ReceivePower` burn prefixes consume the funded quiescent from input-side deliveries, so the cell credits exactly the granted share (vanilla credited the delivered quiescent into the cell as free charge).
...
- **Wind-turbine first-read latch** (`WindTurbineOutputLatchPatches.cs`, decision §0.18c): the same latch pattern on `WindTurbineGenerator.GetGeneratedPower` (covers the Large variant, which does not override it). Per-class decompile verdicts (0.2.6403.27689) picked the latch set: the wind turbine RECOMPUTES its output on every call from `WindStrength` (a static rewritten by `UpdateWind()` in `GameManager.Update` every frame on the main thread), the main-thread `WeatherManager` storm state, and the world atmosphere pressure, so its output can step mid-tick exactly like solar. Every other producer class needs NO latch: gas / solid-fuel generators, the Stirling engine, and the wall turbine return fields written only in `OnAtmosphericTick` (sequential with the electricity tick inside the same GameTick chain, never concurrent), and the RTG returns a constant field. A latch is not free (it swallows the method's side effects on repeat reads), so only mid-tick-mutable outputs are latched.
...
### 8.8 Self-diagnostics: conservation check, shortfall classification, unknown-bridge census, partial-power sentinel, delivery audits, and the auditor round

The diagnostic surfaces below watch the allocator and its enforcement seams. None of them feeds back into any decision: allocation math, ordering, and cache contents are untouched by all of them.
...
**Partial-power sentinel** (`PartialPowerSentinel.cs`, always-on, no config entry; same posture as the ledger audit, §10.7). The no-partial-power contract detector: at the ENFORCE tail, after every ApplyState has settled, it reads each network's final `_powerRatio` and counts a violation on any network that is (1) in this tick's `ShortfallDiagnostics` snapshot, (2) classified Served, and (3) ratio-deprivable: it has at least one plain (non-segmenter) power device, or a storage charge request this tick. The third gate is derived from vanilla ApplyState: the ratio's only effects are the `usedPower *= _powerRatio` billing scale and the power-on/off ladder, so only a plain consumer (scaled delivery, then powered off) or a charging store (`Battery.ReceivePower` adds the ratio-scaled chunk stream to `PowerStored`, so delivered charge = grant * ratio) can be deprived of granted power. A network whose every power member is a routed segmenter is inert and OUT of contract: bills and advertises are cache-governed, downstream delivery is governed by each member's own published cache on its output network, and Powered is reconciled at the ENFORCE tail. That excludes every wireless carrier network by construction (membership is the two dish halves; under the billing handshake its mirrors are structurally asymmetric, unclamped receiver drain vs delivery-gate-clamped advertise, so ratio below 1 is its normal conducting state, live-proven inert by a full-rate charge soak at carrier ratio 0.27) and the tower-top hop networks (receiver + transformer only). The violation threshold is `ratio < SubUnityRatioCeiling` (1 minus 4 float ulps, about 4.77e-7 relative): every healthy vanilla path assigns the literal 1f, and the mod's CacheState postfix (§8.0.6) forces 1f whenever supply is within 0.01 W of demand, but that absolute epsilon sits below one operand ulp from roughly 200 kW up (ulp at 1 MW is about 0.0625 W), so megawatt-scale sum rounding can survive the postfix and divide to a quotient one ulp under 1 with no real gap (observed once as net 503275, displayed ratio 1.000000). The 4-ulp band admits at most about 0.5 W per MW of Required, the charge-delivery audit's tolerance class, while the smallest real unfunded class ever observed is quiescent-scale (10 W). Counters are exact and never throttled (violation ticks, violating net-ticks, distinct nets, worst and latest captures with net id / ratio / Required / Potential / tick); one aggregated warning per 600 ticks while new violations arrive; zero violations produce zero log lines. Positive control: the ScenarioRunner `pgp-partial-power-injection` scenario under-advertises one supplier's published totals on an in-scope net for a bounded window and asserts the counters rise and the worst / latest captures name the net, then that recovery silences it. The always-on APC quiescent is funded by the allocator (§8.0.3) and errored transformers report a 0 bill under mitigation, so an idle or errored segmenter can no longer hold a served network one quiescent below its vanilla Required (the 2026-07-07 natural-sun soak finding); tick-scoped read coherence (the solar first-read latch and the emergency-light toggle queue, §8.0.6 material) removes the transition-dip class the same soak exposed.

**Charge-delivery audit** (`ChargeDeliveryAudit.cs`, always-on, no config entry; the same posture). The grant-vs-credit seam detector, the fifth self-diagnostic surface: the conservation checker audits the allocator's GRANTS and the ledger audit and the sentinel audit the vanilla-facing BILLS, but what actually LANDS in a store (a `PowerStored` credit) was unaudited, and a delivery-side adjustment can under- or over-credit with every other invariant green (the APC quiescent-subtraction wrinkle that motivated it). ALLOCATE publishes a per-tick charge-grant snapshot (refId, granted watts, charge-side net, store kind, straight from the softs roster); the delivery side records each store's credited watts ARGUMENT-derived, never as a `PowerStored` field diff (the store fields are float32, so a 230 MJ nuclear bank quantizes at 16 J per ulp and genuine trickle credits read as multiples of 16): the delivered amount after every patch adjustment, through the implementation's own credit gates, clamped by pre-call headroom (`credited = min(delivered, headroom-before)`, so the vanilla PowerMaximum clamp on a near-full bank reads as the truthful credit). Battery and umbilical credits come from Priority.First/Priority.Last observation brackets (`Patches/ChargeDeliveryObservationPatches.cs`, input-network calls only: the umbilical phase-2 crossing passes a null network and never enters, and discharge and the atmos self-drain are different methods); the APC records its credit at the source inside its own `ReceivePowerPatch`. The ENFORCE tail compares credited against granted per store, gated on the charge-side net classifying Served (an unmet net ratio-scales deliveries by design), on the APC fix master for APC entries, and on the battery band `[granted * BatteryChargeEfficiency, granted]` (the configured loss is legitimate; its sub-500 W per-chunk exemption stays inside the band). Tolerance 0.5 W, the conservation-checker basis (kW-scale chunk-assembled sums). A store whose charge gate legitimately closed between grant and delivery (the APC cell's Mode-based `IsCharged` fill edge) is marked moot for the tick (`MarkChargeGateClosed`) rather than tolerated numerically, and a store owned by an unbillable contributor is never granted at all (§8.0.3), so the fault-lockout class the audit first caught (store 464386: granted 22 to 1000 W for exactly one 120-tick dawn lockout, credited 0 every tick, every other detector silent) is impossible by construction. Counters are exact and never throttled (violation store-ticks, affected ticks, distinct stores, worst and latest captures with refId / granted / credited / tick); one aggregated warning per 600 ticks while new anomalies arrive; zero anomalies produce zero lines; cleared on world load. Positive control: the ScenarioRunner `pgp-rearch-suite` drives `IsViolation` with six synthetic fixture cases (`RSD P<n>` lines) and judges the live window through its `delivery=` verdict field (invariant §17.46).

**Discharge-delivery audit** (`DischargeDeliveryAudit.cs`, always-on, the same posture; decision §0.18a). The charge audit's second direction: what LEAVES a store must equal the granted elastic share (`rigidShare + softTopUp` since §9.2). ALLOCATE publishes a per-tick discharge-grant snapshot straight from the elastics roster (refId, granted watts, discharge-side net, store kind; zero-share entries included so an ungranted drain on a Served net is caught); the drain side records each store's drained watts argument-derived through the vanilla drain gates, clamped by the pre-call store (`drained = min(powerUsed, PowerStored-before)`), via Priority.First/Priority.Last brackets on `Battery.UsePower` and both umbilical halves' `UsePower` (`Patches/DischargeDeliveryObservationPatches.cs`, output-network calls only). Charge / discharge same-tick disambiguation is by-method by construction: the charge audit brackets ReceivePower on the input network, this one brackets UsePower on the output network, and the umbilical phase-2 crossing mutates `PowerStored` directly so it can enter neither sum. The ENFORCE tail compares drained against granted per store, gated on the discharge-side net classifying Served (on an unmet net the elastics are overload-tripped to share 0 and vanilla's scaling owns the rest); the band is exact both ways (discharge has no configured loss), tolerance 0.5 W. The APC cell is out of scope by design: vanilla drains it by deferred ledger settlement (`min(PowerStored, _powerProvided)` repays the PREVIOUS tick's shortfall, the load-bearing cell-cover mechanism this mod keeps), so a same-tick comparison is structurally lag-prone there; no APC grant is published and no bracket exists. Counters exact and never throttled (violation ticks, store-ticks, distinct stores, worst / latest captures); one aggregated warning per 600 ticks; zero anomalies produce zero lines; cleared on world load. Positive control: the `pgp-rearch-suite` drives `IsViolation` with four synthetic fixture cases (`RSD D<n>` lines) and folds the live window into its `audits=` verdict field (invariant §17.47).
...
| VARIABLE_VOLTAGE_FAULT | red | `#ff2626` | `Variable Voltage Fault: connected to <ClassName>. A producer connects only to producers and transformers. ({n}s)` |
...
`<ClassName>` in the VVF hover names the offending violator: the device on the producer's network that triggered the producer-isolation walk. Implementation: the PROTECT-phase producer-isolation walk records the violator class name(s) onto the producer's `ProducerFaultRegistry` entry as it marks the producer for VVF. The hover formatter reads it for display. When multiple violators exist on the same network, the registry stores the violator class names in walk-order; the hover formatter renders the first two by walk-order separated by `, `, with a trailing `, ...` if more than two are present. Example renders: `connected to Battery`, `connected to Battery, AreaPowerControl`, `connected to Battery, AreaPowerControl, ...` (each followed by the fixed advisory `A producer connects only to producers and transformers.`). The class name is the C# class short name (`Battery`, `AreaPowerControl`, `PowerTransmitter`, etc.), not the localized prefab display name, so the message is unambiguous for cross-language players reading reports.
...
33. SoftSupplyShareCache propagates the full §7.3 elastic share (`rigidShare + softTopUp`, §9.2) into ENFORCE via Harmony postfixes on `Battery.GetGeneratedPower`, `AreaPowerControl.GetGeneratedPower`, `RocketPowerUmbilicalMale.GetGeneratedPower`, `RocketPowerUmbilicalFemale.GetGeneratedPower`. Without these postfixes the allocator's elastic decisions do not reach vanilla `_powerRatio` math, and rigid devices on undersupplied output nets partial-power; with them, the donor's soft-funded discharge is advertised on its output net so vanilla drains exactly the granted transfer. The postfixes are not optional.
...
49. The Powered-set conformance assert reads ZERO violations on a healthy build (§8.8): every segmenter published HEALTHY reads `Powered == true` at the ENFORCE tail past a one-tick marshal grace. A violation means the presentation policy's two halves (§10.6) diverged from the allocator's health verdict.
