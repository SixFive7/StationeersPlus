...
| Undersupplied (info) | no flash | `#d9a441` title | `Undersupplied` | `Needs 1.2 kW while upstream delivers 800 W` (needs red, delivers blue), then `Check Medium Transformer` naming the strongest unlocked feeder into the net (line absent when the net has no feeder); keyed by the device's power NETWORK (`UndersuppliedRegistry`, decision 33 (g)), so every device on a DEAD_UNMET net shows the same numbers |
