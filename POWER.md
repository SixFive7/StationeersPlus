...
**Tick-duration watchdog** (`TickDurationWatchdog.cs`, always-on, the same posture; decision §0.18b). Stopwatch spans around the whole atomic tick body and the `RunAtomic` call inside it, judged against a derived threshold `min(max(8 * rolling median, 50 ms), 400 ms)`: the 8x multiple flags decisively pathological ticks on any save size, the 50 ms floor (10 percent of the 500 ms tick period) keeps sub-millisecond-median scheduling noise silent, and the 400 ms ceiling (80 percent of the period) trips unconditionally even when the median itself has degraded. The median comes from a 256-tick ring recomputed every 64 ticks (ceiling-only during the 64-sample warm-up), the ring updates after the comparison so a violating tick never softens its own threshold, and the whole path allocates nothing per tick. Exact counters plus high-water captures for both spans; one aggregated warning per 600 ticks; cleared on world load (invariant §17.48).
...
48. The tick-duration watchdog reads ZERO violations on a healthy build (§8.8): no atomic tick may exceed `min(max(8 * rolling median, 50 ms), 400 ms)` wall-clock. A violation is a performance regression in the mod (the tick owns a 500 ms period at 2 Hz), never a player problem.
