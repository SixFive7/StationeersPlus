...
**Powered Presentation:**

| Setting | Default | Order | Purpose |
|---|---|---|---|
| `DecouplePoweredFromOnOff` | true | 10 | Section `Server - Powered Presentation`, key `Decouple Powered From On Off`. Powered means "this device's network is energized", independent of the device's own switch: a switched-off device on a live net reads IC10 `Power` = 1 while drawing nothing; off restores the vanilla OnOff coupling (decision §0.19). The Powered-ownership behavior itself is always on (decision §0.23); this setting only selects the semantic. |
...
- `EnableTransformerExploitMitigation`, `EnableAreaPowerControlFix`, and `Enable Device Powered Ownership` (decision §0.23: the three behaviors are unconditional; only `Decouple Powered From On Off` remains in the Powered Presentation group).
