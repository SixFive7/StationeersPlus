...
### Resolved decisions (2026-07-18): the fault-presentation package (decision 35)

35. **The fault-presentation package: complete faces on every dark net, capability-driven breakdowns, all-YES hover surfaces, synced burn reasons, the quarantine face, and the APC LED flash.** Built in two verified slices on user approval, closing every gap the 2026-07-18 hover-surface review found. (a) `APC Battery Discharge Rate` defaults to 0 = match the connected output wire (the cell fills the wire's tier cap, never overloads it; a positive value caps below; vanilla had no discharge limit at all, so the mod-introduced cap now follows the wiring instead of silently breaking vanilla bases). (b) The dark-net faces are complete on all sources: DEAD_UNMET nets wear the amber Undersupplied block and DEAD_NOSUPPLY nets that want power the grey No-power-source block, both rendering needs versus the network total plus the always-rendered component line (upstream + generation + storage, zeros included) and a feeder pointer that falls back to a LOCKED feeder (the store-in-lockout room names its APC); consumerless dark nets stay silent; snapshot kind 6 widened with the face byte and the components. (c) Capability-driven source presentation: the APC always shows both parts, Battery / Umbilical only the storage component, Transformer / Link the upstream-only available (`capW - storageW`, Short-by against it; the payload keeps the lossless net-level cap); the transformer dial's deprioritized row is registry-driven with the live countdown. (d) The capability rebuilds: D-04, the wireless carrier network gets plan-row readouts from the pair's settled totals (PowerGridPlus-owned so PowerTransmitterPlus needs no change and both mods work alone or together); D-09, a network that drops out of the plan zeroes its readouts instead of freezing them. (e) Quarantined devices wear a grey info block on casing and button ("This device fought the grid model and was handed back to vanilla control"), synced as the entries-only snapshot kind 7 with the keep-alive TTL; checked first among the info states since a quarantined device is outside the model. (f) The APC fault flash owns the charge LED during lockouts: targeted through the serialized `_chargingLedMaterialChanger` renderer (no name matching), vanilla `ApcMaterialChanger.RefreshState` suppressed while faulted, a forced refresh on exit resumes the correct charge state, and the lever is fully retired as a flash target ("Lever" left the name-fallback list). (g) Burn reasons became a complete surface: fuse blows record a reason before the break, legacy wreckage renders "Burned: cause predates this mod's records", reasons sync to clients (a one-shot message on burn plus the new final join-suffix block, with the stale-cache clear on join fixing the cross-world re-attach bug), and the cursor-pipeline choke point (a NormalModeThing bracket flag plus a Tooltip.HandleToolTipDisplay prefix) completes every crosshair tooltip additively: the mod block is appended when the pipeline missed it (the tool-in-hand branches) and an empty Title is filled so the display gate passes, giving every mod hover all-YES visibility on casing with and without ALT and with any tool in hand, without removing or reordering any vanilla text. (implemented 2026-07-18)

### Resolved decisions (2026-07-17): Powered-ownership carve-outs (decision 34)

34. **Three Powered-ownership carve-outs: rocket umbilical halves owned on their input-network row, the elevator exemption removed, and the umbilical Female enrolled in the allocator despite OnOff.** All three close the same structural gap: retiring vanilla ApplyState (decision 24) removed the last vanilla tick-path Powered writer, so any device the ownership layer skipped kept a frozen flag forever. (a) Umbilical halves. Both halves are IsSegmenter rows the presentation roster never carries (they enroll in the allocator as buffered stores, not routed segs), so nothing wrote their Powered flag and the vanilla working gate (`Powered && OnOff && Error == 0`) could never pass; found live 2026-07-14 as a wired, switched-on, error-free Male stuck at Powered=false with the connect interaction refusing "not powered" forever. The PoweredOwnership sweep now owns each half as a plain device on its INPUT-network row (the charging side, the same row the quiescent bill funds), skipping the PowerCableNetwork single-owner primary test, whose port mapping is unreliable for the two-port halves. Dedicated-server-verified on the 2026-07-14 20:25 save: Powered flipped true within a tick of load (the in-game connect click stays on the playtest list). (b) ElevatorShaft / ElevatorLevel are no longer exempt (amends decision 19's exemption list, which named the elevator family). They are plain Devices with no Powered override and no writer of their own, so the exemption froze every level's flag at its load value: levels powered before the rearchitecture stayed powered, newer ones stayed dark, and because the ElevatorShaftNetwork aggregate requires a CABLED level with Powered=true, frozen-false levels dead-locked whole elevators. The shaft-network aggregate and the per-physics-frame carriage sync only READ the flag, so the sweep owns it safely; mid-shaft pieces without a cable legitimately stay unpowered. Dedicated-server-verified on the user's 2026-07-14 21:57 save: level 673375 reads Powered=true, 124 of 302 elevator pieces powered (exactly the cabled levels). (c) RocketPowerUmbilicalFemale enrolls in the allocator regardless of the snapshot OnOff. The Female is OnOff-BLIND in vanilla (her GetUsedPower has no OnOff gate; the pair's switch is the Male; the QuiescentBill Female branch encodes the same), but the segmenter enrollment gate dropped every switched-off segmenter, so a docked rocket's only supply never enrolled, the rocket-internal net judged DEAD_NOSUPPLY, and the rocket batteries' charge shares were zeroed at publish; found live 2026-07-14 as StructureBatteryMedium 703648 never charging off Female 614953 with a full cell. Dedicated-server-verified on the same save: the battery charged at the full 4900 W relay rate (93 kJ to 122 kJ over 7 ticks), net verdict LIVE. (implemented 2026-07-14; recorded 2026-07-17)

### Resolved decisions (2026-07-17): the fault redesign (decision 33)

33. **The fault redesign: the overload split by cause, deep-first shedding, sheddable step-ups, the watt-minimizing victim cut, the Undersupplied face, and the fresh-device hold fixes.** Five build phases, all dedicated-server-verified 2026-07-17 (pgp-overload-split-fixture 15/15, pgp-chain-fixture 14/14, pgp-deprioritization-victim-fixture 17/17, pgp-fresh-device-trace tick-exact against the pre-fix baseline). This entry amends §5.2, §8.0.1, §8.0.3, §8.3, §8.4, §8.4.1, §11.1, and §13, decision 19's hold gap note, decision 20's step-up partial-grant wording, decision 24's stage-4 "leaf-first" wording, and decision 30's "two info states" clause. (a) Hold fixes. The liveness verdict tests supply-absence FIRST (no supply classifies DEAD_NOSUPPLY before the Unmet test can classify DEAD_UNMET), so an unfed under-construction cable stub never arms the 60 s hold; and every cable-network merge enqueues its member ids for a hold-table clear drained at the next tick head, so a stale hold cannot ride a surviving ReferenceId onto a powered trunk. Both causes together produced the fresh-device bug (a newly connected device darkened its whole merged net for exactly 120 ticks). (b) The overload 1a/1b split. Structural rule 1a trips only when a network's OWN rigid demand, the machines wired directly to it, exceeds the deliverable cap: shedding can never relieve that, so it stays immediate and grow-only. The forwarded-demand case is rule 1b (`DetectResidualOverload`), evaluated at loop settle where every shed is final: the UNSHEDDABLE residual (own machines plus still-active child claims) against the same cap; when it stamps, the allocation loop re-enters with the sticky flags preserved, so an overload that frees budget still un-sheds its victim the same tick. Both rules sum every not-Locked supplier at `EffCap`, Deprioritized included, so "Device overloaded" always means demand exceeds what the hardware could deliver even fully fed, and a same-tick shed cannot stamp a false sticky overload on a surviving co-supplier (the stamp itself skips Deprioritized / already-Overloaded segs: offline this tick, a double fault is noise). (c) The watt-minimizing victim cut. Within the first priority tier whose combined claims can cover the deficit, the victim set is the SMALLEST TOTAL WATTS whose sum still covers it (exact subset choice within the DP bounds, the legacy walk beyond them; equal claims shed the lowest ReferenceId, the pinned legacy tie-break); a tier that cannot cover sheds wholesale. This kills the mass-outage bias: an 8 kW deficit against a 50 kW branch plus two small rooms sheds the rooms, never the branch. The `DeprioritizeReason` vocabulary collapses to LowerPriority / EqualWattCover with the §11.1 Reason lines matching. (d) Deep-first per-round re-decision. The forward pass no longer decides victims: each round grants with the previous decisions, `DecideVictimsDeepFirst` clears every Deprioritized flag and re-decides leaf-to-source against the settled grants, and a second forward pass settles with the fresh decisions, so `Unmet` and the flows the supply rules read are never a pre-shed transient, and leaf-to-trunk escalation completes within one tick's rounds (previously one level per TICK, each level darkening the source net for a spurious 60 s hold). The backward desire gate now excludes Deprioritized contributors, so a shed branch stops inflating its parents' claims the very next round; re-decidability rides the decide pass's flag clear, and the stranded-inflow clawback stays as the belt for the 2-cycle-union settle. (e) Sheddable step-ups. The categorical step-up victim exclusion (§5.2, a 2026-06-16 axiom whose original supports are both dead) is retired: hop protection is `FeedsActiveSeg` alone, so a terminal step-up branch (a heavy machine net fed by a small step-up) joins leaf-first shedding as the leaf it functionally is, while a trunk step-up stays protected exactly while its subtree is active. Priority becomes the player's backbone control (the Stationpedia deprioritization footer now says to give backbone transformers a high Priority); wireless step-up dish pairs are likewise sheddable, so a starved receiver base ends in the pair's Deprioritized lockout plus DEAD_NOSUPPLY downstream instead of an unexplained hold. A six-agent adversarial review preceded the change; the deciding evidence was protected trunks verifiably stranding whole budgets into verdict-dead nets while innocent leaf siblings took lockouts. Shed patterns on existing bases shift with this: expected-shed baselines recorded before the redesign are not comparable and need re-baselining on the next survey. (f) The battery rate/energy split. Rule 2 (the elastic hit-max) stamps only RATE-bound stores; a store whose binding limit is its remaining stored energy (`Elastic.StoredBound`: stored below the rate side, where the APC's rate side is the configured APC discharge rate bounded by the output cable cap, and a configured 0, the default since 2026-07-18, means the cable cap itself: match the wire, never overload it) is running empty, a supply problem, so it is skipped and its shortfall falls to the DEAD_UNMET verdict. A drained bank no longer takes a red "Device overloaded" blaming the hardware. (g) The Undersupplied face. Every net whose verdict lands DEAD_UNMET is marked in `UndersuppliedRegistry` at the liveness tail: needs W (rigid demand plus active consumers' pulls), delivered W (the avail the verdict was judged against), and the strongest unlocked feeder's ReferenceId. Synced as snapshot kind 6, keyed by NETWORK id, with the DeadInput-style 2-tick keep-alive (not in the join handshake); every device on the net hovers the amber `Undersupplied` info block, needs-versus-delivers plus a `Check {feeder}` pointer. An info state, not a lockout: no flash, no countdown, lowest hover precedence (renders only when nothing more specific applies). This closes decision 19's "DEAD_UNMET has no player-facing cue" gap without a fourth fault kind: a shed branch's Deprioritized hover names the contest, and Undersupplied covers the victimless case. (h) Held-net demand presentation, resolved as designed. During a hold tail the held net's standing claim keeps upstream supply reserved, which is what makes the relight at hold expiry instant; the cost (a live sibling can be shed to fund flow that publishes zero) is the price of the reservation, the sibling's hover names the contest, and the held net wears the Undersupplied face. There is NO trickle floor (user decision, purity over comfort): a trickle-fed pocket shedding its export every 120 ticks is the designed, honest 60-second rhythm; document it, never soften it. (i) Gauge honesty. The write-back zeroes a network's `CurrentLoad` readout when the verdict is not LIVE, so analysers and hovers on a dark net read 0 flow instead of the phantom model flow; `RequiredLoad` / `PotentialLoad` deliberately keep model semantics (forward-looking demand and capability stay meaningful on a dark net), and the internal model flow still feeds fuse and burn logic. (j) The tick-duration watchdog (decision 18b) gains a two-tick cold-start grace after each load boundary: the first tick JIT-compiles the whole allocator pipeline (measured near a second even on a fresh empty world, allocator-dominant, once) and fired the "performance regression" warning on every world load. (implemented 2026-07-17)
...
3. **ALLOCATE.** `PowerAllocator.RunAtomic(currentTick)`, the allocator (§8). GATHER consumes the snapshot rows (per-net rigid demand, generator supply, and the sorted segmenter roster; no vanilla topology or demand read happens here), builds the topological order, iterates the fixed-point loop, and decides every deprioritization, overload, elastic discharge share, and storage-charge (soft) grant. Outputs:
   ...
   - Per-tick full fault-registry snapshots broadcast to clients (one `FaultRegistrySnapshotMessage` per kind: deprioritized / device-overload / cycle / current-mismatch / dead-input / cable-overload / undersupplied, §13), via `PowerAllocator.SyncFaultSnapshots`.
...
- **Every direction is deprioritization-eligible (decision 33 retires the categorical step-up exclusion).** Protection is topological, not directional: a contributor whose output network still feeds an ACTIVE child segmenter is a hop and is protected from victim selection (`FeedsActiveSeg`, §8.3), while a terminal branch competes on Priority and the watt-minimizing cut like any room, whatever its direction. A trunk step-up is therefore protected exactly while its subtree is active, and a terminal step-up (a heavy machine net fed by a small step-up) sheds as the leaf it functionally is.
- **Priority is the player's backbone control.** Give trunk transformers a high Priority and they shed last; the Stationpedia deprioritization footer states this. Direction classification (§5.1) remains data the adapters record (the tier pair drives cable-cap math and diagnostics); it no longer grants or denies shedding eligibility.
...
> `Limited to 3.2 kW of the 50.0 kW maximum by the Setting value` (live `FmtWatts` values; the word `Setting` in Stationpedia logic-field orange `#FF8000`)
> `The dial sets priority instead of power`
> `Use IC10 to set the Setting value`
...
This rule means a player who lays out an under-sized cable carrying a heavy transformer's output sees the transformer trip the Cable overloaded fault, with the hover naming the flow against the wire's rating (`Pushing 12.1 kW onto a wire rated for 5.0 kW`), rather than the cable burning silently in a random spot.
...
4. **Shared tail** (§8.0.4): dead-input cue, lockout commit (with the elastic-overload network retry), elastic-share pass (§7.3), the per-net liveness verdict (decision §0.19) with the DEAD_UNMET Undersupplied marking (decision 33 (g)), and the publish tail that writes the share caches, the per-contributor presentation totals for every routed seg kind, the audit grant snapshots, the Powered-presentation, control, and shortfall snapshots, the write-back plan, and runs the conservation check (§8.8). Storage-charge grants are decided inside DECIDE's forward sweep (§9); the tail only publishes them. Every published surface is zeroed for a DEAD net at the source (decision §0.20).
...
- **DEPRIORITIZED is re-decidable.** `DecideVictimsDeepFirst` clears `seg.Deprioritized` for every contributor at the top of each round's decide pass and re-decides leaf-to-source against the round's settled grants (decision 33). So a deprioritization that another device's overload later frees the budget for is retracted within the same tick; an unnecessary deprioritization never freezes for 60 s.

Together these realize the fault precedence **CYCLE > CURRENT-MISMATCH > CABLE-OVERLOADED > DEVICE-OVERLOADED > DEPRIORITIZED** (decision §0.3 as amended by §0.29): rule 1a runs before the decide pass each round on the OWN-demand case shedding can never relieve, and rule 1b catches the forwarded-demand case at loop settle, re-entering the loop when it stamps so relieved budget still re-decides the same tick (§8.0.3, decision 33). Either way a structurally-overcommitted supplier is diagnosed OVERLOAD, the higher-precedence fault, never mislabeled as input-starved.
...
Each round runs six steps in this fixed order (the grant-decide-settle round, decision 33):

1. **`BackwardDesirePass`** (leaf -> source, over the reversed topo order), carrying BOTH flow classes in one walk as a (rigid, soft) demand vector. RIGID: each contributor's `DesiredThroughput` is its share of its output network's residual rigid need (`RigidDemand + consumer pulls - GenSupply`, floored at 0; priority tier DESC, proportional by `EffCap` within a tier, capped at `EffCap`, §8.3.2); `DesiredPull` is the matching input-side draw (`throughput * max(InputDrawFactor, 1) + UsedPower`). An idle contributor whose class bills its quiescent whenever ON (`QuiescentAlwaysOn`, today only the APC, §7.5) presents a bare quiescent-only `DesiredPull = UsedPower`; every other idle contributor presents 0. SOFT: the network's soft desire (`BillableSoftRequestLocal + active consumers' SoftDesiredPull`) splits over the SAME suppliers with the same tier-first proportional splitter, but each contributor's soft capacity is its residual headroom `EffCap - DesiredThroughput`, so soft never displaces rigid capacity; `SoftDesiredPull = SoftDesiredThroughput * max(InputDrawFactor, 1)` plus the quiescent draw ONLY when the contributor carries no rigid pull (the quiescent is carried exactly once, on the rigid pull when any rigid flows, else on the soft pull; in the forward pass the soft pull carries only the remainder the GRANTED rigid pull left uncovered, `max(0, UsedPower - Pull)`, so the seg invariant stays exact even when a quiescent-bearing rigid pull is granted partially). The soft split runs independently of the rigid residual: a net with zero rigid residual still routes charge. Eligibility: BOTH classes gate on the full active test (`!Locked && !Overloaded && !Deprioritized`; decision 33 (d) aligned the rigid gate `DesireActive` with `IsActive`), so a shed branch stops inflating its parents' claims the very next round instead of holding the trunk's claim at the pre-shed level. Re-decidability is not lost: the decide pass (step 4) clears every Deprioritized flag before re-deciding, which re-enters every claim into its net's contest each round. A LOCAL store's request follows the same rule through its OWNER (`SoftOwnerBillable`): GATHER refuses enrollment while the owner sits in a fault lockout, the per-round billable-desire sums exclude an owner that is deprioritized or overloads inside the loop (so no upstream soft inflow is ever sized for it), and the forward grant assigns such a store a zero share; the lockout enforcement zeroes an unbillable owner's bill on every published surface, so any grant here could never be billed or delivered (the 464386 finding, §8.8). Generators are subtracted first; elastic storage is the documented last resort (§7.3) and is not modelled in this pass (it absorbs the per-net shortfall in the forward sweep / elastic-share pass, matching the gen -> transformer -> battery supply order; its LEFTOVER likewise enters only the forward pass's soft pool, §9.2, so the backward pass needed no change for the elastic-to-soft rung: soft desires are demand-sized, and funding is a forward-pass concern).

2. **`DetectStructuralOverload`** (rule 1a, decision 33 (b): OWN demand only). A network whose OWN rigid demand, the machines wired directly to it (`RigidDemand` alone, no forwarded child claims), exceeds `GenSupply + AvailableElastic + the sum of its not-Locked suppliers' EffCap` overloads its `Setting`-limited suppliers (input-limited PT pairs included, taken offline). Shedding can never relieve own demand, so this case stays immediate and grow-only. The cap side counts every not-Locked supplier at `EffCap`, Deprioritized and already-Overloaded included, so the condition keeps re-detecting rather than oscillating and a same-tick shed of one supplier cannot make the net look capacity-short to its surviving co-supplier. The STAMP itself skips Deprioritized / already-Overloaded segs (offline this tick, a double fault is noise), cable-limited suppliers (`CapSetting > CableCap`, rule 3's case), and APCs (no rating to hit). This is the DEVICE_OVERLOADED fault kind (decision §0.29); the shared stamp writes the net-level payload (residual demand, deliverable cap, the elastic slice) to every supplier it flags. The forwarded-demand case is rule 1b, `DetectResidualOverload`, evaluated at loop settle (see below): the UNSHEDDABLE residual (own machines plus still-active child claims) against the same cap, so a trunk faults only when even full shedding cannot fit its residual.

3. **`ForwardSupplyAndDeprioritize`** (source -> leaf, over the topo order, so every supplier's `Throughput` is already final). For each network it computes the supply actually arriving (`firmIn = GenSupply + active suppliers' Throughput`, plus `AvailableElastic`) and `budget = avail - RigidDemand`. The pass DECIDES nothing (decision 33 (d) moved victim choice to step 4, run between this grant pass and the settle re-run): the currently-active consumers are granted highest-priority-first, and each contributor's exact `Throughput`, `Pull`, and the per-net `InflowCommitted` / `PullsGranted` / `RigidServed` / `Unmet` are written. AFTER the rigid grants, SOFT (storage charge) is granted per network from the §9.2 funding ladder, consumed in order: (1) the local firm residual `firmIn - RigidDemand - granted rigid pulls` floored at 0, (2) the soft inflow arriving through the net's active suppliers (granted when THEIR input nets were processed earlier in topo order), and (3) the net's ELASTIC LEFTOVER `AvailableElastic - max(0, RigidDemand + granted rigid pulls - firmIn)` floored at 0 (the lowest rung: eligible discharge capacity the rigid settlement did not consume, tapped only for the soft shortfall firm supply could not cover). The whole pool is capped by the weakest cable's remaining headroom (`CableMax.WeakestCapOnNetwork - (RigidServed + granted rigid pulls)`). One ratio `softRatio = min(1, softAvail / softDemand)` scales every local charge request and every active consumer's soft pull on the net; a consumer's granted soft throughput is further capped at its remaining headroom `EffCap - Throughput`, and a deprioritized / locked / overloaded consumer gets zero soft. The per-net `SoftGrantedLocal` / `SoftPullsGranted` / `ElasticFundedSoft` (the granted soft the firm pool could not cover, measured against actual granted totals) and per-seg `SoftThrough` / `SoftPull` are written. Deprioritization decisions, budgets, and `Unmet` never see the soft class; unmet soft desire is silently clamped (§9.6), and the leftover is a pure function of the round's rigid settlement, so the new rung adds no oscillation mode to the fixed point.

4. **`DecideVictimsDeepFirst`** (leaf -> source, decision 33 (d)). Clears every seg's Deprioritized flag (the clear IS the re-decision mechanism), then per network, walking deepest first so a parent's `FeedsActiveSeg` sees its children's decisions from THIS round: with `avail` and `budget` recomputed from the just-settled grants, if the active consumers' claims exceed the budget the victim set comes from `SelectDeprioritizationVictims` (§8.3 / §8.3.3: wholesale below the covering tier, the smallest-total-watts cut within it) and is marked whole (never partial) with the decision-time hover payload (needs, upstream demand, upstream supply, shortfall-at-cut, reason, victim priority). A hop, a consumer whose output net still feeds active child segs (`FeedsActiveSeg`), is protected; the categorical step-up exclusion is retired (§5.2). A network with no supply at all (`avail <= Eps`) decides nothing (dead-input idle, §8.3.1). Deep-first ordering is what completes leaf-to-trunk escalation within one tick's rounds instead of one level per tick.

5. **`ForwardSupplyAndDeprioritize` again** (the settle pass): re-grants with the fresh decisions, so `Unmet`, `PullsGranted`, and the flows the supply rules read are never a pre-shed transient (the double-forward round, decision 33 (d)).

6. **`DetectSupplyOverload`** (after the settle pass, because both rules read the settled `Unmet` / `PullsGranted`). Two rules: the elastic hit-max (a network still `Unmet` after gen + inflow + full elastic discharge trips its live RATE-bound elastics; a `StoredBound` store, whose binding limit is remaining stored energy rather than rate hardware, is skipped and its shortfall falls to the DEAD_UNMET verdict and the Undersupplied face, decision 33 (f); §8.4.1, capacity family, `OverloadRegistry`, shared payload computed at the cohort commit), and the §5.7 cable overflow (flow above the weakest cable cap with generators alone under it trips every supplier + elastic on the network instead of burning the cable; the CABLE_OVERLOADED kind, decision §0.29: the rule stamps the kind bit plus the (flow, weakest-cable cap) payload here, and the commit routes to `CableOverloadRegistry`).

The **grant-decide-settle round with rule 1a up front** is load-bearing: 1a trips the unshiftable own-demand case before any decision could mislabel it, the decide pass re-decides against honestly settled grants, and the settle pass re-grants before the supply rules read `Unmet`, so a shed that relieves a network is honoured within the SAME round. Combined with grow-only overload (§8.0.1), this kills the deprioritization <-> overload 2-cycle (§8.0.7).

**Convergence.** After the four passes, the `(deprioritized, overload, elastic-overload)` RefId sets are collected. If they equal the previous round's sets, the loop has converged. If they equal the round-before-previous sets, the loop is in a 2-cycle between two states: the intermediate state's flags are OR-ed in (a safe superset, never under-protective), then one `BackwardDesirePass` + `ForwardSupplyAndDeprioritize` settles throughputs (no decide pass runs after the union, so the OR-ed flags are settled without being re-decided), and the loop exits. The round cap is `2 * segs.Count + 4`; exhausting it logs a `LogWarning` and keeps the last settled state (internally consistent and safe, possibly not minimal). Only deprioritization can oscillate (overload is monotonic), so the 2-cycle guard plus the union fallback is sufficient.

**The rule-1b outer iteration.** After the loop converges, `DetectResidualOverload` (rule 1b, decision 33 (b)) tests every network's unsheddable residual (own machines plus still-active child claims) against the hardware cap; if it stamps anyone, `RunAtomic` re-enters `RunAllocationLoop` with the sticky flags preserved (bounded at `segs.Count + 1` passes), so the budget a new overload frees re-decides, and a victim that was shed to protect a now-offline trunk is un-shed the SAME tick (the 2c guarantee, decision 33 (g)). Capacity is tested before the residual is left to the availability story: a too-small trunk reads "upgrade the transformer" while scarcity keeps reading "add supply".

**The stranded-inflow clawback.** Since decision 33 (d) the backward desire gate excludes Deprioritized contributors, so the steady-state strand source is gone; what remains is the 2-cycle union settle (flags OR-ed in and settled without a fresh decide) and the rule-1b re-entry, where a settled grant can still fund a claim a later pass takes offline: committed and billed upstream, taken by nobody. After the loop, the lockout commits, the dead-input rebuild, and the shortfall census (all of which read the deciding state), any tick that deprioritized at least one contributor runs a clawback in `RunAtomic`: walking leaf to source, every network whose total committed inflow (rigid plus soft) exceeds its total consumption (served demand, granted pulls, charge, and soft pulls; soft counts because the soft stage funds charge from the rigid firm residual) takes the surplus back from its active suppliers in reverse grant order, shrinking each seg's published throughput and pull together (a fully clawed always-on-quiescent seg keeps its funded quiescent pull, `QuiescentAlwaysOn`, since vanilla bills that draw regardless of throughput) and landing the pull reduction on the supplier's input network before that network is visited. There is no re-split and no re-grant (a full settle re-pass was tried and re-granted the freed budget to other branches, reshaping real allocation on trip ticks), so every seg off the stranded chains keeps its deciding-pass numbers to the bit and the only real-world change is that nothing is billed upstream for power nobody consumes. No decision is re-opened.
...
- **Deprioritization victim selection** (`SelectDeprioritizationVictims`, §8.3): tier-major watt-minimizing (decision 33) over `(priority ASC, claim DESC quantised to whole Watts, ReferenceId ASC)`. Tiers go priority ASC; a tier whose combined claims cannot cover the remaining deficit is shed wholesale, and the first tier that CAN cover ends selection with the victim set of the smallest total watts whose sum still covers it (exact 0/1 subset choice via a kept-watts subset-sum DP within bounds, the legacy best-fit-decreasing walk beyond them; on equal claims the lowest `ReferenceId` is shed). Claims floor to whole Watts via `(int)Math.Floor`; the deficit rounds up past the Eps tolerance via `(int)Math.Ceiling(deficit - Eps)`, so the selected set always restores claims under budget in float terms (never an under-selection that would trip the §8.4 elastic hit-max) at the cost of at most one extra small victim on sub-Watt boundaries. Every comparison is integer / `ReferenceId` only, and the selector is a pure static function of (candidates, deficit), pinned by ScenarioRunner's `pgp-deprioritization-victim-fixture` synthetic cases (§8.3.3). It replaced the flat `(priority ASC, ReferenceId ASC)` walk, whose over-deprioritization cost §8.3.3 documents.
...
Deprioritization is decided in `DecideVictimsDeepFirst` (§8.0.3 step 4), per input network, leaf-to-source against the round's settled grants, and re-decided every round (§8.0.1). For a network whose budget the active consumers' claims exceed:
...
2. If the sum of active consumers' claims (`DesiredPull`) exceeds the budget, `SelectDeprioritizationVictims` picks the victim set for the whole deficit in one pass (tier-major watt-minimizing, §8.0.5 / §8.3.3, decision 33): lowest priority tier first, shed wholesale while the tier cannot cover; the first tier that can cover contributes the smallest-total-watts cut. Every victim is deprioritized whole (never partial).
3. Hops are never victims: a contributor whose output network still feeds an ACTIVE child segmenter is protected (`FeedsActiveSeg`, §5.2; decision 33 retired the categorical step-up exclusion, so protection is topological only). If only protected consumers remain, the budget is accepted as-is.
...
### 8.4 Overload: the capacity rules (1a own demand, 1b unsheddable residual)

The capacity fault is net-level since decision 33, two rules sharing one stamp. **Rule 1a** (per round, immediate): a network's OWN rigid demand, the machines wired directly to it, exceeds the deliverable cap `GenSupply + AvailableElastic + sum of not-Locked suppliers' EffCap`; shedding can never relieve own demand. **Rule 1b** (at loop settle, re-entering the loop when it stamps, §8.0.3): the network's UNSHEDDABLE residual, its own machines plus its still-active child segmenters' claims, exceeds the same cap after every shed is final; a trunk faults only when even full leaf-first shedding cannot fit its residual under the cap. Both stamp every `Setting`-limited, not-Locked, not-yet-stamped supplier of the tripping net (a Deprioritized supplier keeps its `EffCap` in the cap sum but is skipped by the stamp; a cable-limited supplier, `CapSetting > CableCap`, belongs to rule 3; the APC has no rating to hit). This is the DEVICE_OVERLOADED fault kind (decision §0.29): the hover title is `Transformer overloaded fault` on a transformer and `Link overloaded fault` on the wireless pair, over the demand-versus-available diagnostics, and the payload is the rule's net-level locals (residual demand, deliverable cap, the elastic slice). Note: a player who reduced Setting below OutputMaximum via IC10 will see overload fire at the lower threshold; this is intentional (they explicitly asked the transformer to throttle below its rated max).

Examples (assuming Setting = OutputMaximum at default):

- T1 (Setting 500 W) and T2 (Setting 500 W) feed an output network with rigid demand 1500 W. The deliverable cap is 1000 W; the demand cannot fit even fully fed, and both Setting-limited suppliers are stamped.
- T1 (cable-limited: its output cable caps it at 200 W of its 500 W Setting) and T2 (Setting 500 W) feed an output network with rigid demand 900 W. The cap is 700 W; the demand cannot fit, and only T2 is stamped: T1's binding limit is its cable, not its Setting, so it belongs to rule 3 (cable overflow), never the capacity stamp.
- IC10 example: a player IC10 script writes `Setting = 200` on T1 to throttle it. T1's `EffCap` drops to 200 - UsedPower; if downstream rigid demand exceeds the resulting cap, T1 is stamped. The player can later raise Setting back to OutputMaximum to clear the constraint.

Rationale for stamping the Setting-limited suppliers: the player-facing diagnostic is "the hardware feeding this net cannot deliver what it demands, upgrade or split it." A cable-limited supplier is not the culprit of a capacity shortfall (its wire is, rule 3's story), and a Deprioritized supplier is already offline with its own explanation.
...
### 8.3.3 Deprioritization victim selection example (watt-minimizing)
...
`SelectDeprioritizationVictims` (tier-major watt-minimizing, §8.0.5, decision 33) chooses the smallest-total-watts cut that covers the 1000 W deficit: T_b (1000) alone is minimal, so T_b is deprioritized and selection ends. T_a and T_c survive; one device is deprioritized where the earlier flat (priority ASC, ReferenceId ASC) walk deprioritized T_a (500) and then T_b (1000), two victims where one sufficed. Where a single cover is NOT minimal the cut wins: claims 50000 / 6000 / 5000 against an 8000 W deficit shed the 6000 and the 5000 (11000 W total), never the 50000 (the pre-decision-33 smallest-single-cover rule's failure shape).

A tier whose combined claims cannot cover the deficit is shed wholesale and selection moves to the next tier; within a covering tier, when no smaller subset reaches the deficit the whole tier is the minimal cut and sheds (claims 500 and 700 against a 1000 W deficit shed both: neither alone reaches 1000). The policy and both ReferenceId tie rules are pinned by ScenarioRunner's `pgp-deprioritization-victim-fixture` synthetic cases, which drive the pure selector directly with the exact numbers above.
...
Five distinct failure modes (after the current-mismatch addition in §11.5 and the §0.29 overload split). Colour identifies the family of cause; the hover block identifies the specific cause: a merged state-plus-title line with a live countdown over one or more diagnostics lines (live watts on the two overload kinds and the deprioritized kind; fixed sentences elsewhere). Wording locked by §0.30.

### 11.1 Colour and hover text inventory

| State | Flash | Hex | Title (line 1 of the block) | Diagnostics line(s) |
|---|---|---|---|---|
| DEPRIORITIZED | orange | `#ffa500` band | `Deprioritized fault` | `Upstream demand of 30.0 kW exceeded the upstream supply of 20.0 kW` (demand red, supply blue), then `Due to the shortfall of 10.0 kW this device's 12.0 kW share was cut` (shortfall red, share grey), then one to three grey `Reason: ...` lines chosen by the recorded cause (see below) |
| DEVICE_OVERLOADED | red | `#ff2626` | `Transformer overloaded fault` / `Link overloaded fault` / `Battery overloaded fault` / `APC overloaded fault` / `Umbilical overloaded fault` (per hovered device, see below) | `Downstream demand of 150.0 kW exceeds the available 100.0 kW` (demand red, available blue), then the source-breakdown driven by device CAPABILITY, never by which value is currently zero (user decision 2026-07-18): the APC (can pass through AND store) always shows `70.0 kW upstream + 30.0 kW internal storage = 100.0 kW available` (upstream blue, internal storage teal, total blue), zero-valued parts included; a storage-only device (Battery / Umbilical) shows `30.0 kW of the available comes from internal storage` (teal); a pass-through-only device (Transformer / Link) shows no breakdown; then `Short by 50.0 kW` (red) |
| CABLE_OVERLOADED | red | `#ff2626` | `Cable overloaded fault` | `Pushing 12.1 kW onto a wire rated for 5.0 kW` (flow red, rating blue), then `Overloaded by 7.1 kW` (red) |
| CYCLE_FAULT | red | `#ff2626` | `Cycle fault` | `This device is part of a power loop that feeds back into itself` |
| CURRENT_MISMATCH_FAULT | red | `#ff2626` | `Current mismatch fault` | `Put a transformer between the generator DC and the AC grid`, then `Incompatible devices on this network:`, then the offending devices one per line as ` - {name}` (pretty prefab name red, bullet grey); four or fewer shown in full, more than four shown as the first three plus a grey ` - And N more devices...` row |
| No upstream supply (info) | no flash | grey title | `No upstream supply` | `No power is reaching this device from upstream` |
| Throttled (info) | no flash | `#d9a441` title | `Throttled` | `Limited to 3.2 kW of the 50.0 kW maximum by the Setting value` (setting red, maximum blue, the word `Setting` in Stationpedia logic-field orange `#FF8000`), then `The dial sets priority instead of power`, then `Use IC10 to set the Setting value` |
| Undersupplied (info) | no flash | `#d9a441` title | `Undersupplied` | `Needs 1.2 kW while the network raises 800 W` (needs red, total blue), then the always-rendered component line `500 W upstream + 300 W generation + 0 W storage = 800 W` (upstream and generation blue, storage teal; zeros included per the 2026-07-18 complete-on-all-sources decision), then `Check Medium Transformer` naming the strongest feeder into the net, locked feeders included as fallback (line absent when the net has no feeder); keyed by the device's power NETWORK (`UndersuppliedRegistry`, decision 33 (g)), so every device on a DEAD_UNMET net shows the same numbers |
| No power source (info) | no flash | grey title | `No power source` | The DEAD_NOSUPPLY face (2026-07-18): same layout as Undersupplied (needs, the always-rendered component line, the feeder pointer) with the total normally 0; covers the store-in-lockout room whose only supply sits in a fault window. Only nets that WANT power are marked; an empty wire run shows nothing |
| Quarantined (info) | no flash | grey title | `Quarantined` | `This device fought the grid model and was handed back to vanilla control` (decision 35 (e)); per-device, checked FIRST among the info states; synced as the entries-only snapshot kind 7 |

Every state renders ONE merged block, identical on the casing tooltip and the on/off button's title box. Line 1 is `{On|Off} - {title}: {n}s`: the switch word green `#00FF00` when on / grey `#9aa0a6` when off, the dash grey, the fault title and countdown red `#ff2626`; switchless devices (no `HasOnOffState`) drop the `{On|Off} - ` prefix; the three info states use their grey / amber titles and carry NO countdown. Info precedence is No upstream supply, then Throttled, then Undersupplied (the most generic, rendered only when nothing more specific applies). The diagnostics line(s) below are calm-grey `#9aa0a6` prose with the coloured values nested inside, one learnable rule across every block: fault red `#ff2626` = power wanted (downstream demand, cable flow, the shortfall / overage) and the offending device names; informational blue `#008AE6` = power available (upstream supply, device capacity, wire rating, throttle maximum); teal `#2DD4BF` = the internal-storage (battery) contribution split out from the available total; orange `#FF8000` = the IC10 `Setting` field, the exact colour Stationpedia paints a logic field. Exactly one block per hover: lower-precedence faults are not stacked (§11.5) and the info states are suppressed while any fault is active (the throttle note no longer stacks under a fault). The button's Action box keeps the pure vanilla action word.
...
The diagnostics lines carry live watt values from the fault's registry payload. Device overload reads `(valueW, capW, storageW)`: `Downstream demand of 150.0 kW exceeds the available 100.0 kW`, the source breakdown (upstream = `capW - storageW`, floored at 0), and `Short by 50.0 kW` (= `valueW - capW`); the breakdown is selected by the device class's capability (`OverloadSourceMode`), never by the current values, so the hover teaches the device's mechanics even when a component is 0: the APC always renders both parts (`0 W upstream + 1.0 kW internal storage = 1.0 kW available` on a dead input), Battery / Umbilical render only the storage component (`1.0 kW of the available comes from internal storage`), and Transformer / Link render no breakdown (no storage component exists on the device; note the total can still include OTHER stores on the net via the 1a/1b elastic slice, an accepted simplification). Cable overload reads `(flowW, capW)`: `Pushing 12.1 kW onto a wire rated for 5.0 kW` then `Overloaded by 7.1 kW` (= `flowW - capW`). Deprioritized reads `(needsW, upstreamDemandW, upstreamSupplyW, shortfallW, reason, victimPriority)`: `Upstream demand of 30.0 kW exceeded the upstream supply of 20.0 kW`, then `Due to the shortfall of 10.0 kW this device's 12.0 kW share was cut`, then the `Reason:` line(s). Watt values format as whole watts below 1000 W and as kilowatts with one decimal at or above (`FmtWatts`), locale decimal separator like the countdown. All payloads come from the registry entry, recorded at the detection site and mirrored to clients (§13).

The deprioritized `shortfallW` and `reason` are recorded at the exact instant the allocator sheds the device, not recomputed at hover time, because deprioritization is iterative: within one solve the victim selector sheds non-covering tiers wholesale (each victim facing a declining remaining shortfall) and ends with a covering cut whose members share the shortfall the cut was chosen against, and across ticks the net's numbers move. `shortfallW` is therefore the remaining upstream deficit at THIS device's cut, which can be smaller than `upstreamDemandW - upstreamSupplyW`. The `Reason:` line(s) come from the recorded `DeprioritizeReason`: LowerPriority renders `Reason: Its priority of {victimPriority} was below other consumers` (the device lost purely on priority, no same-priority peer survived); EqualWattCover renders `Reason: Its priority was equal to other consumers` + `Reason: It was part of the smallest combined cut covering the shortfall` (decision 33: the device's tier could cover the remaining deficit and this device was part of the smallest-total-watts cut chosen to do so; the retired EqualBestFit / EqualLargest pair collapsed into this one case).
...
Implementation: `FaultHover.TryGetMergedBlock` is the single renderer for all five faults plus the three info states; the legacy per-line renderer (`TryGetLine`) and its merged-layout gate (`UsesMergedLayout`) are deleted. The hovered instance is threaded in from both hover patches (`FaultHoverPatches` body tooltip, `FaultButtonTooltipPatches` button title box; the Action box is untouched vanilla). `FaultHoverPatches` runs an outermost-only depth guard (prefix + finalizer pair): several targets sit on one inheritance chain whose derived overrides call `base.GetPassiveTooltip`, Harmony detours base calls like virtual dispatch, and without the guard one hover poll appended the fault block at every patched level (the two-or-three-copies defect fixed with §0.29).

The current-mismatch block leads with the fixed advisory `Put a transformer between the generator DC and the AC grid`, then the header `Incompatible devices on this network:`, then the offending devices. Implementation: the PROTECT-phase producer-isolation walk records the violator name(s) onto the producer's `CurrentMismatchFaultRegistry` entry as it marks the producer, deduplicated in walk-order and capped: four or fewer distinct names are all carried, more than four carries the first three plus a `+N` overflow marker. The hover formatter renders each name as a ` - {name}` bullet (name red, bullet grey) and the `+N` marker as a grey ` - And N more devices...` row. The name is the prettified prefab display name (the `PrefabName` with its category prefix stripped and a space inserted before each interior capital, e.g. `StructureWallCooler` -> `Wall Cooler`), captured on the snapshot `DeviceRow` at build time so the off-main-thread detector never reads a live Thing; it is the device TYPE, not a player's rename, so the list stays stable and dedups cleanly.
...
- Fault state is broadcast to clients as ONE per-tick full-snapshot message, `FaultRegistrySnapshotMessage`, with a `Kind` byte selecting the registry (deprioritized = 0, device-overload = 1, cycle = 2, current-mismatch = 3, dead-input = 4, cable-overload = 5, undersupplied = 6, quarantined = 7 with entries only). This single message replaces the former per-transition diff messages (one per registry) outright (deviation P16); transitions are implicit in the snapshot deltas. Per-entry wire layout by kind: every kind carries `(int64 refId, int32 remainingTicks)`; deprioritized (0) appends `(single needsW, single upstreamDemandW, single upstreamSupplyW)` plus the reason byte and victim priority (§11.1, decisions §0.30 and 33 (c)); the two overload kinds (1 and 5) append their hover-diagnostics payload (§11.1, decision §0.29); current-mismatch appends the violator-names string; undersupplied (6) appends `(byte face, single needsW, single upstreamW, single genW, single storageW, int64 feederRefId)` (face 0 = Undersupplied / 1 = No power source; the 2026-07-18 all-sources widening) and its entries carry NETWORK ReferenceIds with a keep-alive TTL, not device ids (decision 33 (g)).
