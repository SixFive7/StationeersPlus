...
The deprioritized `shortfallW` and `reason` are recorded at the exact instant the allocator sheds the device, not recomputed at hover time, because deprioritization is iterative: within one solve the victim selector cuts devices one at a time (largest-first subtracts and repeats, best-fit ends it), so each victim faced a different remaining shortfall, and across ticks the net's numbers move. `shortfallW` is therefore the remaining upstream deficit at THIS device's cut, which can be smaller than `upstreamDemandW - upstreamSupplyW`. The `Reason:` line(s) come from the recorded `DeprioritizeReason`: LowerPriority renders `Reason: Its priority of {victimPriority} was below other consumers` (the device lost purely on priority, no same-priority peer survived); EqualBestFit renders `Reason: Its priority was equal to other consumers` + `Reason: Its draw alone covered the remaining shortfall` (a same-priority peer survived and this device's draw alone closed the gap, rule 2a); EqualLargest renders `Reason: Its priority was equal to other consumers` + `Reason: No single consumer's draw could cover the shortfall` + `Reason: Its draw was the largest consumer` (a same-priority peer survived, no single device covered the gap, so the largest was cut, rule 2b).
