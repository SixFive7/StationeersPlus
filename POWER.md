...
| DEVICE_OVERLOADED | red | `#ff2626` | `Transformer overloaded fault` / `Link overloaded fault` / `Battery overloaded fault` / `APC overloaded fault` / `Umbilical overloaded fault` (per hovered device, see below) | `Downstream demand of 150.0 kW exceeds the available 100.0 kW` (demand red, available blue), then the breakdown `70.0 kW upstream + 30.0 kW internal storage = 100.0 kW available` (upstream blue, internal storage teal, total blue) ONLY when storage contributes and upstream is non-zero, then `Short by 50.0 kW` (red) |
...
The diagnostics lines carry live watt values from the fault's registry payload. Device overload reads `(valueW, capW, storageW)`: `Downstream demand of 150.0 kW exceeds the available 100.0 kW`, the breakdown `70.0 kW upstream + 30.0 kW internal storage = 100.0 kW available` (upstream = `capW - storageW`), and `Short by 50.0 kW` (= `valueW - capW`); the breakdown line is dropped when `storageW` is zero or equals the cap, so a device with no storage on its net never shows a phantom `0 W internal storage`, and a single-source cap is not restated as a trivial `X = X`. Cable overload reads `(flowW, capW)`: `Pushing 12.1 kW onto a wire rated for 5.0 kW` then `Overloaded by 7.1 kW` (= `flowW - capW`). Deprioritized reads `(needsW, upstreamDemandW, upstreamSupplyW, shortfallW, reason, victimPriority)`: `Upstream demand of 30.0 kW exceeded the upstream supply of 20.0 kW`, then `Due to the shortfall of 10.0 kW this device's 12.0 kW share was cut`, then the `Reason:` line(s). Watt values format as whole watts below 1000 W and as kilowatts with one decimal at or above (`FmtWatts`), locale decimal separator like the countdown. All payloads come from the registry entry, recorded at the detection site and mirrored to clients (§13).
