...
**Conflict detection.** The mod replaces Color Cycler and Network Painter. It cannot coexist with them because they patch the same methods. `BepInIncompatibility` attributes cover load-time detection, but StationeersLaunchPad loads mods progressively, so those assemblies may not exist when `Awake()` runs. A second check runs on `Prefab.OnPrefabsLoaded`, scanning `AppDomain.CurrentDomain.GetAssemblies()` for the conflicting assembly names. If found, the mod logs a fatal error and starts a coroutine that repeats a red console banner every 5 seconds, bounded to `ConflictBannerRepeats` (6) with the last line saying it is the last. No Harmony patches are applied. See [../../Research/Patterns/ConflictDetection.md](../../Research/Patterns/ConflictDetection.md) for the general assembly-scan-on-prefab-load pattern.

**Player-facing output goes through the shared helper.** Nothing in the mod calls `ConsoleWindow` or `UnityEngine.Debug` directly. Every console line and every player-facing log line routes through `StationeersPlus.Shared.PlayerMessage`, linked into the build from `Patterns/Console/PlayerMessage.cs` and initialised once in `Awake` with the display name `Spray Paint Plus`, which is what supplies the bracketed prefix. The helper owns the console mechanics that are easy to get wrong (the inverted `aged` flag, the missing `PrintWarning`, `PrintError`'s stack trace, and the rule that `Debug.LogError` is re-printed by the console itself, lowercased and with a trace nothing can suppress). Throttling is per call site and mandatory: the console has no rate limiting and every print is an O(1024) array shift. Three call sites use `Throttle.Never` deliberately, each because something else already bounds them: the conflict banner (its `for` loop is the bound), `WarningNotifier.Print` (its callers count, see section 4.4), and the eyedropper cross-family line (a mouse-down edge answering one deliberate action).
