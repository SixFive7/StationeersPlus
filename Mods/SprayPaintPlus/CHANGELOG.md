...
- NEW: Every capability now has a client setting and a server setting, and works only when both allow it. 33 settings across nine groups, up from 18 across five. Your half decides what you do, never what you see; the host's half decides what the world allows.
...
- REMOVED: the Extra Paintable Structures setting, because the base game now ships Steel Frame (Corner) and (Side) paintable as of game version 0.2.6332.27293.
- CHANGE: Glow Paint already needed both sides to agree, but a mismatch failed silently and read as a bug. It now says so in its descriptions and reports it in-game.
