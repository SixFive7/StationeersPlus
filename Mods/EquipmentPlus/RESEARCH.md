...
- Upstream writes `Device.SetLogicValue(LogicType, double)` directly on the client. On a remote MP client the write applies locally and is overwritten on next sync. Upstream is effectively host-only. We route via `SetLogicFromClient` to fix this.
