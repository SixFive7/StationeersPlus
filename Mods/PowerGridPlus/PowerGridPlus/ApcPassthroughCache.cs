...
namespace PowerGridPlus
{
    ...
    ///     <see cref="Research"/> Device.md "_powerProvided one-tick lag"). The allocator sizes upstream
    ///     supply to the APC's CURRENT pull, so billing a stale <c>_powerProvided</c> on the input leaves
    ///     the input network short by the one-tick demand change (the net-503288 flicker). The
    ...
    ///     a read older than one tick reports no fresh value (the APC bills 0 passthrough that tick until
    ///     the allocator roster includes it again).</para>
    /// </summary>
