...
{
    // CURRENT_MISMATCH_FAULT lockout state machine for power producers
    ...
    //
    // Client mirror model identical to DeprioritizedRegistry (per-tick full
    ...
    internal static class CurrentMismatchFaultRegistry
    {
        ...
        internal static void NoteCurrentMismatchFault(long referenceId, int currentTick, string violatorNames)
        {
        ...
        internal static bool IsCurrentMismatchFaulted(long referenceId, int currentTick)
        {
            if (IsClientPeer)
