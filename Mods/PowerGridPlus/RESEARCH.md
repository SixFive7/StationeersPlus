...
- **`Patches/PassthroughTopology.cs`** (extended) -- adds `GetBridgeNetworks` / `DirtyBridgeNetworks` (the unified per-device dirty helper, covering Transformer / Battery / PowerTransmitter / PowerReceiver) used by the write paths and the dish-link refresh; `GetOtherSide` / `AnyPassthroughEnabled` now read `PassthroughSettingsSync.Effective*` instead of `Settings.Enable*.Value` directly.
...
- `Patches/PassthroughTopology.cs` (new) holds the shared helpers. `GetOtherSide(device, from)` is the per-device bridge test (server toggle + per-device mode), lifted verbatim from the old inline merge, so which devices bridge is unchanged. `GatherReachable(start)` is a stack BFS over the bridge graph with a `HashSet<CableNetwork>` visited set; it returns every reachable network except `start`, ordered by `ReferenceId` so the merge order is identical on every peer. The visited set makes a cyclic bridge graph terminate (each network folded in once).
