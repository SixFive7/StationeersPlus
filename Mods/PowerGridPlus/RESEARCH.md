...
> **2026-06-10 rework (v0.2.0).** The inner-tick architecture changed: the Re-Volt-derived `PowerGridTick` subclass, its routing prefixes, and `DevicePatches.AssessPower` are DELETED. Phases 1 and 3 of the atomic tick now run the unmodified vanilla `PowerTick`; every PowerGridPlus behaviour is delivered through device-method patches plus the atomic phases. Where an older paragraph below mentions `PowerGridTick`, proportional Re-Volt load sharing, the sliding-window burn, the `Setting`-to-`Priority` redirect, or the `ImportQuantity` / `ExportQuantity` repurpose, the authoritative current design is this section plus `POWER.md` at the repo root.
...
The atomic tick (`Patches/AtomicElectricityTickPatch.cs`) owns `ElectricityManager.ElectricityTick` via prefix-return-false and runs five phases per tick:

1. **OBSERVE**: vanilla `PowerTick.Initialise + CalculateState` per network (fresh `Required` / `Potential`).
2. **Phase 1.5**: server-side OFF-as-reset sweep (`OffAsResetSweep.cs`), wrong-tier cable burns (`VoltageTierEnforcer.cs`), CYCLE_FAULT detection (`CycleGraphBuilder.cs`, directed-SCC over `SegmentingDeviceRegistry.cs`), producer-isolation VARIABLE_VOLTAGE_FAULT (`VariableVoltageFaultDetector.cs` + `ProducerClassifier.cs`, with a cable-burn fallback for unclassified producer-like devices), and a re-observe when anything newly faulted.
3. **DECIDE**: `PowerAllocator.cs` -- the joint shed / overload fixed-point iteration over every segmenting device (transformers, APCs, linked PT/PR pairs as one contributor each, batteries and rocket umbilicals as elastic suppliers / soft demanders), the elastic discharge shares (`SoftSupplyShareCache.cs`), and the single-pass surplus walk for charge shares (`SoftDemandShareCache.cs`). Integer-only sort keys for MP determinism. Then the per-tick full fault snapshots to clients (`FaultRegistrySnapshotMessage.cs`).
4. **ENFORCE**: vanilla `Initialise + CalculateState + ApplyState` per network; the device-method patches make the allocator's decisions visible (lockout zeroes via `Patches/CycleFaultEnforcementPatches.cs` + `Patches/TransformerExploitPatches.cs` + `Patches/ProducerFaultEnforcementPatches.cs`; share caps via the battery / APC / umbilical patches). Vanilla `_powerRatio` partial scaling stays as the safety net.
5. **Per-device OnPowerTick + CircuitHolders.Execute**: vanilla copies.
...
- **Stationary battery rate limits + elastic shares** (`Patches/StationaryBatteryPatches.cs`, `Patches/BatteryLogicPatches.cs`). Per-prefab absolute Watts caps on `GetUsedPower` / `GetGeneratedPower` plus the allocator share caps; re-implements `Battery.ReceivePower` with a charge-efficiency factor and a low-power-trickle floor. Exposes `MaxChargeSpeed` / `MaxDischargeSpeed` / `ChargeSpeed` / `DischargeSpeed` (6583-6586); the prior `ImportQuantity` / `ExportQuantity` repurpose is removed.
- **Transformer / APC fixes** (`Patches/TransformerExploitPatches.cs`, `Patches/TransformerLogicPatches.cs`, `Patches/AreaPowerControlPatches.cs`). Closes the transformer free-power exploit with the vanilla Setting-based formulas, restores quiescent draw, exposes `PowerActual`, closes the APC's idle power leak, applies APC cable caps + soft-power LogicTypes. `Patches/TransformerSettingInitPatch.cs` initialises `Setting = OutputMaximum` on new construction (load order makes saved values win for loaded worlds).
- **Rocket umbilical support** (`Patches/RocketUmbilicalPatches.cs`). Rate caps + the four soft-power LogicTypes on `StructurePowerUmbilicalMale` / `StructurePowerUmbilicalFemale` (the Female exposes them through the `Device`-base declarations with an instance filter; the slot-keyed overloads force explicit `typeof(LogicType)` argument arrays on those patches).
...
- **`Settings.cs`** -- every `ConfigEntry<T>` the mod binds. Section names follow the repo convention (`Server - <Topic>`); `(Server-authoritative)` description prefix; `("Order", int)` tags for in-panel ordering. Sections: `Server - Cable Simulation`, `Server - Cable Costs`, `Server - Voltage Tiers`, `Server - Batteries`, `Server - Transformers`, `Server - Area Power Control`, `Server - Power Transmitters`.
...
- **`Patches/CableNetworkPatches.cs`** (extended) -- besides injecting the `PowerGridTick`, the `DirtyPowerAndDataDeviceLists` postfix propagates the dirty across the passthrough component AND drives the device-list refresh cascade. `ScheduleCascade` / `ScheduleCascadeForDevice` / `RefreshAllNetworks` enqueue (via the shared `MainThreadDispatcher`) a main-thread pass that, per network, diffs a `ConditionalWeakTable`-cached signature of the merged `DataDeviceList` and, when it changed, calls `OnDeviceConnectToNetwork` on every physical member (the vanilla consumer-refresh signal) -- refreshing motherboards (via the host `Computer`), IC housings, and `LogicBase` sensors. Fired from the propagation postfix (transitive far networks), the `SetLogicValue` write paths (the flipped device's component), the dish-link patches, and the settings-sync.
...
Every Harmony patch, with target method, prefix / postfix / reverse, gating config, and a one-line behaviour summary. `Patches/` holds 19 files: 18 Harmony patch classes plus `CableCostPatches.cs`, a plain helper invoked from `Plugin.OnPrefabsLoaded` (listed below for completeness). The two `INetworkMessage` types (`PassthroughModeMessage`, `PassthroughSettingsMessage`) and `PassthroughSettingsSync` live in the mod root, not `Patches/`, and are covered in section 6.
...
The mod registers two LaunchPadBooster `INetworkMessage` types (`PassthroughModeMessage`, `PassthroughSettingsMessage`) and an `IJoinSuffixSerializer` (all added with the device-list refresh work). The rest of its state reaches clients through vanilla state-sync:
...
The decision log spans three named phases (the 2026-05-12 first build pass, the 2026-05-13 round-2 follow-up that resolved every stubbed / deferred item, and the 2026-05-13 round-3 device-rejection -> reactive-cable-burn pivot) plus the dated entries that follow: the 2026-05-14 LogicPassthroughMode addition, the 2026-05-15 transformer tier-pair refinement, and the 2026-05-18 passthrough extension and merge-determinism fix. Entries are append-only; supersedes are explicit.
