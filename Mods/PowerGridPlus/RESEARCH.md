...
Every Harmony patch, with target method, prefix / postfix / reverse, gating config, and a one-line behaviour summary. `Patches/` holds 18 files: 17 Harmony patch classes plus `CableCostPatches.cs`, a plain helper invoked from `Plugin.OnPrefabsLoaded` (listed below for completeness).
...
| `TransformerPassthroughLogicPatches` | `Transformer.SetLogicValue` | prefix | always (server-only) | Server-only (`NetworkManager.IsServer`); writes the mode to `PassthroughModeStore`, dirties both side networks' data device lists. |
...
| `InheritedLogicablePassthroughLogicPatches` | `Device.SetLogicValue` | prefix | always (server-only) | Server-only; writes the mode and dirties the device's two networks (Battery Input/Output; TX `InputNetwork`+`LinkedReceiver?.OutputNetwork`; RX `OutputNetwork`+`LinkedPowerTransmitter?.InputNetwork`). |
...
- `LogicPassthroughMode` is a per-device mode (on `Transformer`, `Battery`, `PowerTransmitter`, and `PowerReceiver`) stored host-side in `PassthroughModeStore` and serialised into the side-car at save time. A joining client does not directly read the host's store; instead, the host's data-device-list merge (the actual passthrough behaviour) runs server-side, and the resulting `DataDeviceList` membership is what clients observe when their IC10 chips iterate the list. The `SetLogicValue` prefixes (both the `Transformer` override and the base-`Device` patch) are guarded by `NetworkManager.IsServer`, so a client tablet write does not mutate the client's local store; it routes through the host's authoritative logic-write path, and the read on any client is consistent within the next tick.
