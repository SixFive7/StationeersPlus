...
- **Bridging devices can be made logic-transparent.** A transformer, stationary battery, or Area Power Controller splits a power network into two sides, and normally an IC10 chip or logic reader on one side cannot see the devices on the other. Turn passthrough on and that wall disappears for logic: a reader sees through the bridge, and through a whole chain of bridges, so devices several transformers away still read as one logic network, with no extra data cable. Looped and branching layouts are handled safely. A linked Microwave Power Transmitter and Receiver pair bridges the same way across its wireless link.
