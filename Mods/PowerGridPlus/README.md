...
- **Deprioritized fault** (orange flash): a transformer (or other bridging device) cannot get its full draw from its input network. When siblings compete for a short input, the lowest Priority is deprioritized first. While deprioritized it contributes 0 to its output network, so the subnet goes dark cleanly instead of every device flickering. The hover shows the contest (`Upstream demand of 30.0 kW exceeded the upstream supply of 20.0 kW`, the shortfall that cut this device's share, and a `Reason:` line naming why it was picked: a lower priority than its siblings, or membership of the smallest combined cut covering the shortfall).
- **Device overloaded fault** (red flash): the draw on a network exceeds what its supplying device can deliver. The hover titles the actual device (`Transformer overloaded fault`, `Link overloaded fault` on a wireless dish pair, `Battery overloaded fault`, `APC overloaded fault`, `Umbilical overloaded fault`) and shows the live numbers (`Downstream demand of 150.0 kW exceeds the available 100.0 kW`, a storage breakdown line when a battery contributes to the available total, then `Short by 50.0 kW`), so you know what to upgrade or split. A storage device at its full effective discharge rate with demand still unmet takes the same fault under its own name.
- **Cable overloaded fault** (red flash): the devices could deliver the flow, but the network's weakest cable cannot carry it, so the suppliers go offline instead of burning the cable. The hover shows the flow against the wire's rating (`Pushing 80.0 kW onto a wire rated for 5.0 kW`, then `Overloaded by 75.0 kW`). A battery tripped this way takes the same visible lockout (it previously produced zero silently, with no cue).
- **Cycle fault** (red flash): the device is part of a closed power loop (`This device is part of a power loop that feeds back into itself`). Every bridging device on the loop faults and contributes 0, which dissolves the loop. No cable is burned for loops.
...
- Not every dark subnet is a fault; three info states explain the rest, with no flash, no countdown, and no lockout. A contributor whose input network has no power source at all simply idles with a steady grey `No upstream supply` hover (`No power is reaching this device from upstream`); power its input and it delivers again immediately. A transformer throttled below its rated throughput by an IC10 `Setting` write carries an amber `Throttled` note (`Limited to 3.2 kW of the 50.0 kW maximum by the Setting value`, `The dial sets priority instead of power`, `Use IC10 to set the Setting value`). And a network whose demand outruns what reaches it, with shedding exhausted, marks every device on it with an amber `Undersupplied` block (`Needs 1.2 kW while upstream delivers 800 W`, plus `Check {feeder}` naming the strongest feeder into the net), so a dark room explains itself even when no single device is at fault.
...
- **The in-world knob sets Priority instead** (non-negative integer, no upper cap, default 100, step 10 per screwdriver click or 1 with Alt). The Labeller tool also sets Priority. Priority decides who is deprioritized first when an input network runs short; comparisons are local to each input network. Give backbone transformers a high Priority so they shed last.
...
- **Blueprint prints cannot corrupt the wiring.** A wrong-tier cable that arrives without the build cursor (a blueprint print, a creative spawn, another mod placing cables) is refused on the spot: the existing wiring stays intact and targetable, the refused cable drops as its construction kit, and the console (F3) prints one "Illegal cable placement" line with the coordinates.
- **Corrupted mixed-tier wiring self-heals.** A save that already carries two cables of different tiers stacked in one spot (old prints) repairs itself when power touches the network: the higher-tier cable survives and the lower-tier one is removed cleanly, no burnt wreckage; a genuine wrong-tier junction still burns at the tier boundary. Every repair or burn prints one "Illegal mixed-tier cable" console line with the coordinates.
- **Every world load sweeps up stacked leftovers.** Burnt-cable wreckage sharing a spot with a live cable, and hidden duplicate cables left by old blueprint prints, are removed once per load, one console line each. A lone piece of burnt wreckage is untouched; it stays as the normal "a cable burned here" cue.
