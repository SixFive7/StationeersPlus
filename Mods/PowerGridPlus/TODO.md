...
- **Shed APC still bills its quiescent unfunded (known residual, low frequency).** A SHED AreaPowerControl bills its 10 W quiescent through `AreaPowerControlPatches.GetUsedPowerPatch` (the vanilla OnOff-plus-output gate is true) while the allocator grants a shed seg nothing, so a net that sheds an APC yet stays Served could hold one quiescent below vanilla Required and trip the partial-power sentinel. Not exercised by the natural-sun soaks (Luna's boot sheds hit transformers). Sketched fix: gate the patch's quiescent on the published `TransformerSupplyCache` totals, transformer-style, so a shed APC bills 0 like every other inactive contributor.
