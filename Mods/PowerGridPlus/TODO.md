...
- **Shed victim policy: deliberate claim-size tiebreak is an open design decision.** `ShedVictimOrder` is (priority ASC, ReferenceId ASC); the spec's historical claim-DESC tiebreak never worked (it compared the granted `Pull`, always zero at victim-selection time). The straightforward live-claim fix (`DesiredPull` DESC) is a player-visible change in which device sheds (largest-claim-first can lock one large feed over a small transient deficit where the shipped order spreads across small victims), and a controlled A/B showed boot shed races draw different victims per run, so cross-boot trajectory comparison cannot validate a selection change. A considered policy (for example smallest-claim-first that still covers the deficit, or deficit-proportional selection) needs its own design pass plus purpose-built evidence (same-boot A/B or synthetic fixtures) before shipping; see POWER.md section 8.0.5 and the `ShedVictimOrder` comment.

- **Cold-start shed trap (open defect, low frequency).** Shed and overload lockouts fire instantly on detection with no hysteresis or arming window. On a cold start (a just-loaded world whose upstream supply has not yet propagated through a deep chain, or a large same-tick topology change) a contributor can shed on transiently wrong numbers and freeze its subnet for 60 seconds. The dead-input rule covers the no-supply-at-all case, but partial-supply transients can still trip. Sketched fix: arm the shed / overload detectors per network only after a short run of clean ticks. Not yet implemented; noted during the 2026-07-02 rearchitecture design review.

## Feature ideas (not yet decided)
