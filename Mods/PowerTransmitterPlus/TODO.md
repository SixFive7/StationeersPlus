...
## Reply to Maximus's Steam comment: wireless data-network bridging request

Maximus left a feature request on the Power Transmitter Plus Workshop page (9 June 2026, https://steamcommunity.com/sharedfiles/filedetails/?id=3707677512) asking whether the transmitter dish could double as a wireless data connector that bridges separate data networks, the way the Satisfactory FicsIt Networks mod's Wireless Access Point links networks. Still unanswered (the last author replies were 1 June). Draft and post a reply. Before answering, check whether the sibling mod [Power Grid Plus](../PowerGridPlus/) already covers this: its logic-passthrough merge makes the receiver-cable-side network see the transmitter-cable-side devices across the dish link (see the "Client-side LinkedPowerTransmitter mirror (wireless link symmetry)" item below). If that satisfies the request, point Maximus at Power Grid Plus; otherwise scope it as a possible feature or explain why it is out of scope.
...
| [CHANGELOG.md](CHANGELOG.md) | New top entry at the next minor version (visible visual overhaul + new settings; v1.9.0 already shipped as the mod API + billing fixes release). No BepInEx reset caveat: only adding fields, not renaming existing ones. |
...
- Dedicated server driving procedure: [TestRig/DedicatedServer/CLAUDE.md](../../TestRig/DedicatedServer/CLAUDE.md). Shared rig conventions: [TestRig/CLAUDE.md](../../TestRig/CLAUDE.md). Session lock (one lock covers the dedicated server and the client rig): [TestRig/session.lock.template](../../TestRig/session.lock.template)
