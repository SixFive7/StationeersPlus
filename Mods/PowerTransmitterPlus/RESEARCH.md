...
`VisualiserPatches.cs` - Single Harmony patch on `WirelessPower.VisualizerIntensity` setter. Postfix: if `__instance is PowerTransmitter`, call `BeamManager.SetLineIntensity(...)`. This fires from the ThreadPool worker; safe because `SetLineIntensity` enqueues to the dispatcher. Drives pulse-train scroll speed only; does NOT drive show / hide (see `BeamVisibility.cs` and `BeamManager.ReevaluateVisibility`).

`LinkVisibilityPatch.cs` - Single Harmony Postfix on `PowerTransmitter.LinkedReceiver` property setter. Calls `BeamManager.ReevaluateVisibility(__instance, "Link")` on every transition. The full predicate runs there; if the new value made the link null, `ShouldShow` returns false and the beam hides; if the new value is non-null and both dishes are on with valid aim, the beam shows. Vanilla's setter body is gated by `value != _linkedReceiver` so it only runs once per actual change, but Harmony postfixes fire unconditionally, so `ReevaluateVisibility` is idempotent. Fires on both server (via `TryContactReceiver`, `OnDestroy`) and remote clients (via `ProcessUpdate`, which assigns through the property when the `NetworkUpdateType.WirelessPower.Receiver` bit is set), so one Postfix covers every link-transition path on every peer.

`OnOffPatches.cs` - Single Harmony Postfix on `Thing.OnInteractableUpdated(Interactable)`. Filtered to `interactable.Action == InteractableType.OnOff && __instance is WirelessPower`, then resolves to the owning transmitter: self if `PowerTransmitter`, `r.LinkedPowerTransmitter` if `PowerReceiver`. Calls `BeamManager.ReevaluateVisibility(tx, "OnOff")`. Patching the base `Thing` virtual catches both subclasses because every override in the chain calls `base.OnInteractableUpdated(interactable)` first (`Device.OnInteractableUpdated` and the dish overrides at `PowerTransmitter` / `PowerReceiver`). The `Action == OnOff` filter is mandatory: `Powered`, `Mode`, every button, every slot all enter the same callback on their own changes. Fires on every peer (server, single-player host, remote clients via `Thing.ProcessInteractableUpdate` -> `Interact(state, false)` -> the setter -> callback), and only on actual transitions because the power tick's `OnServer.Interact(InteractPowered, ...)` calls are caller-gated with `if (Powered)` / `if (!Powered)`. See `Research/GameClasses/Interactable.md` for the full propagation map. Background: this is the missing v1.5.1 piece. Pre-v1.5.1 the beam followed `VisualizerIntensity` (which vanilla zeroes on `OnOff = false`), so an off dish naturally hid its beam. v1.5.1 moved show / hide onto `LinkedReceiver` (which vanilla does NOT null on `OnOff = false`), so the beam needed an explicit OnOff gate; this patch provides it.
...
- `Dictionary<long, WeakReference<WirelessPower>> _tracked`: parallel id-keyed registry for enumeration at save / join / post-load passes.
- `[ThreadStatic] bool _suppressReset`: re-entry flag set during our own servo writes.
- `MaxAimIterations = 10` (inner), `MaxOuterIterations = 5`, `AimConvergenceTolerance = 0.01f` (1 cm).
- `HandleWrite(dish, newId)`:
  ...
  - If target is a `WirelessPower`: run the joint mutual-aim solve. Seed both sides canonically from `SeedFromRootDirection` (root-to-root direction inverse-mapped to dish-local H/V), then alternate `SolveAim(dish, ...)` and `SolveAim(partner, ...)` per outer iteration, predicting each side's ray endpoint via `PredictEndpoint` virtually. Write only this dish's (H, V).
  - Otherwise (non-dish target): single-side `SolveAim` against `target.transform.position`.
  - Commit (H, V) to `RotatableBehaviour.TargetHorizontal` / `TargetVertical` under the suppression flag.
- `ResolveCachedTarget(dish, targetId)`: re-enters the solve for an already-cached entry. Used by the post-load pass; clears the cache (as if the user had cleared it) when `Thing.Find` returns null.

`SeedFromRootDirection(dish, targetWorld)`: returns `(H, V)` mapping the dish-local direction of `(targetWorld - dish.transform.position)`. Both endpoints are rotation-invariant, so the seed is identical regardless of the dish's prior aim history. Both sides of a joint solve seed the same way for any given pair, making the converged answer order-independent.

`SolveAim(dish, toPos, h, v)`: inner fixed-point loop. Seeds origin from `PredictEndpoint(dish, dish.RayTransform, h, v)` (canonical seed pose, not current `RayTransform.position`), then iterates `dLocal -> (H, V) -> PredictEndpoint -> origin'` until the origin moves less than 1 cm. Returns the converged (H, V).

`PredictEndpoint(dish, endpoint, h, v)`: temporarily writes `dish.AxleTransform.localRotation` and `dish.DishTransform.localRotation` to the candidate pose, reads `endpoint.position` (Unity recomputes the world chain on read), then restores the saved rotations in a `finally`. `endpoint` is any descendant of `dish.AxleTransform`: pass `dish.RayTransform` to predict the ray origin, or `rx.DishTarget` to predict the partner's link-raycast target point. Side-effect-free at the game level: direct `Transform.localRotation` writes do not flow through the `WirelessPower.Horizontal` / `Vertical` property setters, so they do not set `NetworkUpdateFlags |= 256`, do not call `BeamManager.RefreshIfVisible`, and do not invalidate the servo state. Synchronous Unity transform recomputation only.

Why a joint solve: each dish's `RayTransform` and `DishTarget` are children of the rotating Head, so they move with (H, V). A single-side solve treats the partner's `DishTarget` as static and ends up aimed at where it WAS at solve time, not where it lands after both dishes slew. At long range (150 m+) that residual exceeds the partner's small `DishTarget` collider and the link probe misses entirely. The joint solve eliminates the partner-drift by predicting both poses together. The fixed point is symmetric: each side independently solves the same system and lands on the same shared answer, so the partner does not need to have us cached as its target for the solve to work; if the partner is later configured (or already is), it computes the same fixed point and meets us there. Convergence is mm-level in one outer iteration for typical link distances; combined contraction factor `k_self * k_partner ~ 1e-4` at 200 m.

Post-load pass (`AutoAimSaveLoadPatches.cs`, `GameManagerUpdateThingsOnGameStartAutoAimResolvePatch`):
- Postfix on `GameManager.UpdateThingsOnGameStart` (per `Research/GameClasses/GameManager.md` "Load finalize chain", this is the earliest reliable hook for "every Thing's `OnFinishedLoad` has run").
- Walks `AutoAimState.SnapshotEntries()` materialized into a list, calls `ResolveCachedTarget(dish, targetId)` per entry. Targets that still resolve are re-solved under the current solver (migrates older saves to the joint-aim math without player action, repairs aim after position-edited saves). Targets that no longer resolve get their cache cleared.
- Host-side only via the `!(IsActive && !IsServer)` guard (`Research/GameSystems/NetworkRoles.md`); pure remote clients receive cache and aim from the host via `IJoinSuffixSerializer.DeserializeJoinSuffix` and the live tick stream.
- Logs `Auto-aim post-load: <re-solved> re-solved, <cleared> stale target(s) cleared (out of <count>)` when the cache is non-empty.
...
|---|---|---|---|
| `LinkedReceiverSetterPatch` | `PowerTransmitter.LinkedReceiver` (setter) | Postfix | `BeamManager.ReevaluateVisibility(__instance, "Link")`. Re-runs the full predicate; show / hide based on link + both dishes' `OnOff` + aim. Fires on every peer (server via `TryContactReceiver` / `OnDestroy`, remote clients via `ProcessUpdate`). |
| `ThingOnInteractableUpdatedPatch` | `Thing.OnInteractableUpdated` | Postfix | Filtered to `interactable.Action == InteractableType.OnOff && __instance is WirelessPower`. Resolves `PowerReceiver` to its `LinkedPowerTransmitter`. `BeamManager.ReevaluateVisibility(tx, "OnOff")`. Fires on every peer (server, single-player, remote clients via interactable-state replication) only on actual switch-state transitions. |
| `VisualizerIntensitySetterPatch` | `WirelessPower.VisualizerIntensity` (setter) | Postfix | Cast to `PowerTransmitter`; `BeamManager.SetLineIntensity`. Pulse-train scroll speed only, no show / hide. |
| `WirelessPowerHorizontalSetterPatch` | `WirelessPower.Horizontal` (setter) | Postfix | Resolves both `PowerTransmitter` (self) and `PowerReceiver` (via `LinkedPowerTransmitter`). `BeamManager.ReevaluateVisibility(tx, "Slew")`. Fires per slew step on every peer that simulates rotation. |
| `WirelessPowerVerticalSetterPatch` | `WirelessPower.Vertical` (setter) | Postfix | Same as `Horizontal`. |
...
| # | Patch class | Target | Type |
|---|---|---|---|
| 15 | `WirelessPowerSetLogicValuePatch` | `WirelessPower.SetLogicValue` | Prefix (false for `MicrowaveAutoAimTarget`; passes through for everything else). Joint mutual-aim fixed-point solve for dish-to-dish targets (canonical pivot-to-pivot seed, inner cap 10, outer cap 5, 1 cm tolerance, `PredictEndpoint` and `SolveAim` helpers); single-side fallback for non-dish targets |
...
| 18b | `GameManagerUpdateThingsOnGameStartAutoAimResolvePatch` | `GameManager.UpdateThingsOnGameStart` | Postfix. Walks `AutoAimState._tracked` after every Thing's `OnFinishedLoad` has run, re-solves cached pairs and clears stale targets. Host-side gated on `!(IsActive && !IsServer)` |
...
**Depends on:** [../../Research/GameClasses/PowerTransmitter.md](../../Research/GameClasses/PowerTransmitter.md) (joint mutual-aim geometry, dish transform hierarchy, `TryContactReceiver` raycast, Head-child transforms move with rotation), [../../Research/GameClasses/RotatableBehaviour.md](../../Research/GameClasses/RotatableBehaviour.md) (`TargetHorizontal` / `TargetVertical` setters and the servo delta-state that Auto-aim rides), [../../Research/GameClasses/GameManager.md](../../Research/GameClasses/GameManager.md) (load finalize chain; `UpdateThingsOnGameStart` is the post-load hook), [../../Research/GameSystems/NetworkRoles.md](../../Research/GameSystems/NetworkRoles.md) (host-side guard for the post-load pass), [../../Research/Patterns/ConditionalWeakTableCache.md](../../Research/Patterns/ConditionalWeakTableCache.md) (per-dish cache lifecycle tied to GC).
