...
- `TestRig/` is the test rig that drives Stationeers: one AOT-compiled binary (`TestRig/testrig.exe`, built from `TestRig/src/` and committed), one session lock, two halves (`TestRig/DedicatedServer/`, a self-contained headless server install, and `TestRig/ClientRig/`, N driven game clients running a control-plane plugin), plus the playtest engine reached through `testrig playtest`. The whole tree is gitignored deny-all with a named allowlist for the binary and its sources, the four docs (`CLAUDE.md`, `MANUAL.md`, `RESEARCH.md`, `playtest/CLAUDE.md`), `dev-plugins/` source, and the PowerShell launcher retained as the parity reference. See "Tool: TestRig for driving the game" below.
...
|---|---|---|---|
| `README.md` | Each mod's folder root (committed); also each `dev-plugins/<Name>/` that needs one | End users (GitHub readers, Workshop subscribers) | Feature overview, installation, settings tables, issue reporting, credits, license. Source of truth for user-facing content, mirrored into `About.xml`. |
...
The `README.md` and `RESEARCH.md` rows are scoped "each mod's folder root" because that is where nearly all of them live. Two committed non-mod tools carry equivalents for the same reason and follow the same rules: the test rig, whose operating manual is `TestRig/MANUAL.md` and whose durable internals are `TestRig/RESEARCH.md` (the rig has no `README.md`; its front door is the auto-loading `TestRig/CLAUDE.md`), and each `dev-plugins/<Name>/` that needs one, such as `TestRig/DedicatedServer/dev-plugins/ScenarioRunner/README.md`. They are developer tooling, not mods, so no `About.xml` mirroring, no `CHANGELOG.md`, no `PLAYTEST.md` and no release tag applies to them; everything else in the table's Contents column does. Neither is published to the documentation site (see "Things that do NOT go in `Web/`").
...
- `TestRig/ClientRig/data/<instance>/userdata/`, the per-instance save root each client instance gets through StationeersLaunchPad's `SavePathOverride`. A client instance can be a listen host, so this root holds real worlds under `saves/`, written by `testrig save` (which waits for the game's own confirmation and warns rather than claiming success). `testrig remove` deletes the whole root along with the instance tree, by design, which for a host is the world every joiner was in.
...
Tier 3 is free to edit but not unowned: it belongs to whoever holds the rig session lock (see `TestRig/CLAUDE.md`). Do not clobber another session's save tree or instance. **A world on either half is session-scoped**: one that appears after the session's first mutating command is deleted when that session ends, so stage a save you want to keep BEFORE taking the lock or before that first command. That includes a listen host's world under a client instance's save root, which used to be wiped unconditionally and is now scoped exactly like a dedicated-server world.

The rig has no copy-in or wipe verb for saves; copy save trees in directly with the standard file tools. `testrig start --target server --new <Map>` remains the way to get a brand-new world when no source is provided.

**Three client-half hazards sit on the tier-1 boundary, and an instance that can create a world sharpens all of them.**

- `POST /savepath` retargets `Settings.CurrentData.SavePath` on a **running** client. In the deployed `ClientDriver` it refuses a path inside the developer's real user-data folder only while the caller omits `force=true`. That refusal lives in plugin code, not in any rule an agent reads, and `testrig call --body` passes the body through unread, so an agent that passes `force=true` points a live client's saves straight back at the tier-1 folder. Never pass `force=true` on `/savepath` unless the user asked for exactly that.
...
- An instance whose `SavePathOverride` could not be written (no `stationeers.launchpad.cfg` yet) shares the developer's user-data folder. That is a hard failure for a `--role host` create and a warning for a `--role client` one, because a host creates a world and a client only reads one. Treat the warning as a stop, not a note.

The merged `TestRig` plugin removes both overrides outright, so passing either becomes a 400 that names the removal. It is built and not yet deployed, so until it is, the first two hazards above are live.
...
The decompile-content curation hook fires on five things: a Read of any file ending in `.decompiled.cs` anywhere in the tree (the suffix safety net), a Read or Glob under `.work/decomp/` (the canonical output path), a Glob naming `rocketstation_Data/Managed/` (a direct DLL listing; Read cannot serve here because the Read tool rejects binaries pre-flight, so its `PostToolUse` never fires), a shell command that invokes a decompiler (`ilspycmd`, `ICSharpCode.Decompiler`), and a shell command whose text contains a decompile path or suffix (`.work/decomp/`, `*.decompiled.cs`, `rocketstation_Data/Managed/`). That last case is what catches inspection via `cat`, `grep`, `rg`, `head`, `tail`, `xxd`, `strings`, `Get-Content`, `Select-String` and anything else, without enumerating each tool. All five inject the same Rule 2 reminder pointing at `Research/WORKFLOW.md`.

"Shell" in the last two means both shell tools, `Bash` and `PowerShell`. Covering only one is not partial coverage, it is a hole: `PowerShell` is the primary shell in this environment, so `Get-Content .work\decomp\<ver>\Assembly-CSharp.decompiled.cs` is the likeliest way a decompile gets read here, and it fired nothing at all until the `PowerShell` registration was added. Both tools deliver an identical `tool_input` shape (`command` plus `description`), so the script needed no change to read the second one.

The hook script decides all five for itself by reading `tool_input` from stdin. The `if` rules in `.claude/settings.json` are only a cheap pre-filter, and that split is not optional: Bash `if` rules fail open, so when the harness cannot parse a command (a loop, a heredoc) every Bash `if` rule fires regardless of content. Seven Bash rules used to mean seven copies of the reminder on a `for` loop that had never been near a decompile. The shell registration is now one entry with the matcher `Bash|PowerShell` and no `if` at all, so both shells reach the script and the script alone decides; the path tools carry one `*decomp*` rule each, which also ends the double-fire two overlapping Read rules produced on the canonical path. Registering a shell with no `if` costs a process on every call to it, measured at 210-285 ms (median 219) on the quiet path, which is the price of closing the hole rather than leaving the rule enforced on one shell only. That price was already being paid for Bash. Grep is intentionally not hooked: per the Claude Code source (`GrepTool.preparePermissionMatcher`), Grep `if` rules match against the search regex argument, not the path or glob, so a path-shaped `if` rule on Grep can never fire for normal usage. Reading derived `.cs` content from anywhere else (a Downloads folder, an ad-hoc workspace, any path outside `.work/decomp/`) bypasses curation and is forbidden.
...
`TestRig/` at the repo root is the test rig. **One binary, `TestRig/testrig.exe`, one session lock, one system**, with two halves and a playtest harness inside it:

```
testrig <verb> [--target all|server|clients|<instance>] [options]
```

- The **dedicated server** half (`TestRig/DedicatedServer/`) is a self-contained headless install, driven through its stdin by a wrapper process.
- The **client** half (`TestRig/ClientRig/`) creates and drives N isolated game clients over HTTP through a BepInEx control-plane plugin.
- The **playtest verb** runs a mod's in-game checks against the client half with nobody at the keyboard, and reports pass, fail or inconclusive per check. Asked to run a mod's playtests? Run `testrig playtest` rather than hand-rolling the rig sequence. The checks themselves are C# under `Mods/<Mod>/playtests/`, compiled into the binary.

**The binary is build output and it is committed**, so nothing needs a build step to drive the rig. It embeds a digest of `TestRig/src/` and **exits 7 without acting** when the tree beside it disagrees; rebuild with `dotnet publish TestRig/src/TestRig.Cli/TestRig.Cli.csproj -c Release -r win-x64`, which installs the new binary itself, and commit it in the same commit as the source change. The suite is `dotnet test TestRig/src/TestRig.slnx`. The PowerShell launcher and its libraries are retained under `TestRig/` as the parity reference until the port has driven a real multiplayer playtest end to end; they are known broken and must be read rather than run.

Four documents, and only the first two auto-load: `TestRig/CLAUDE.md` (the rules: the lock, the save tiers, state hygiene, the safety rules; **read it before running anything under `TestRig/`**), `TestRig/playtest/CLAUDE.md` (harness work), `TestRig/MANUAL.md` (the operating reference: every verb, the working sequences, the control-plane endpoint catalogue) and `TestRig/RESEARCH.md` (durable internals). Two nested files cover the code: `TestRig/src/CLAUDE.md` and `TestRig/dev-plugins/TestRig/CLAUDE.md`. Running `testrig.exe` with no verb prints the whole surface, generated from the verb and option tables so it cannot drift, and where a verb does not apply to a target the binary refuses and names one that works.

**Either half can host a world, and picking the wrong one wastes a session.** A client instance created with `--role host` and driven with `POST /host` becomes a listen host: one process that runs the simulation, accepts joiners over loopback RakNet, and plays a character. Use the dedicated server when the test wants a server that is not a player (soak runs, in-process scenario probes, save-edit round trips). Use a listen host when the test needs a host who plays, which the dedicated server cannot provide at all because it has no player character. Game internals: `Research/GameSystems/ListenHost.md`. It is one rig and one lock either way.

The rig is a shared single-instance resource: every agent on this machine contends for one install, because the halves share the developer's one game install and per-Windows-user Unity state (`PlayerCookie-v2.xml`, the `HKCU\Software\Rocketwerkz\rocketstation` PlayerPrefs key) with each other and with the developer's own client. Before driving it you MUST acquire the session lock (`testrig lock --purpose "<reason>"`) and pass `--as <id>` on every mutating command. It spans a whole test session across many start/stop cycles, expires on a heartbeat timer so an idle agent does not starve others, stays live while the rig is busy (a player connected to the server, or any client instance running) and is reclaimable past an absolute idle ceiling even then. Breaking another session's live lock is `--break-lock`, not `--force`, and is human-gated: never do it without the user's explicit say-so. **Releasing the lock restores the rig, and acquiring one restores it again as a backstop**, so your own leftovers, including a world your session created on either half, do not survive your release unless you pass `--keep-state`. Rules: `TestRig/CLAUDE.md`; implementation: `TestRig/src/TestRig.Core/Session/`.

The whole tree is gitignored deny-all with a named allowlist (see the `TestRig` block in `.gitignore`): the binary and its sources under `src/`, the four docs, source under `dev-plugins/`, and the retained PowerShell tree. Deny-all is deliberate, because routine rig actions drop artifacts into the folder (`snapshot --out-file before.json`, `/screenshot?path=`) that must never be committable by accident.

Developer plugins live under `dev-plugins/<Name>/<Name>.sln` with source in `dev-plugins/<Name>/<Name>/`, and none of them ships to the Workshop. Deployed today: `ScenarioRunner` (`TestRig/DedicatedServer/dev-plugins/ScenarioRunner/`) probes the dedicated server from inside, and `ClientDriver` (`TestRig/ClientRig/dev-plugins/ClientDriver/`) is the control plane inside each game client. Built but not yet deployed: `TestRig/dev-plugins/TestRig/`, one plugin that merges both and loads into either half.

The binary reads `<StationeersPath>` from `Directory.Build.props`, `STEAMCMD_PATH` and `STATIONEERS_CLIENTRIG_ROOT` from the environment (set per `DEV.md`). It contains no developer-specific paths.
...
- Mod-local `README.md`, `RESEARCH.md`, or `Plans/` content, and the tooling docs under `TestRig/`. The site publishes the central knowledge base in `Research/` only; mod-local docs stay with the mod for now, and the rig docs describe a local test harness that has no meaning to a public reader.
