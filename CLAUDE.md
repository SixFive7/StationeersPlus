...
- `TestRig/` holds the two tools that drive Stationeers for testing: `TestRig/DedicatedServer/` (a self-contained headless server install) and `TestRig/ClientRig/` (the `ClientDriver` plugin plus its launcher, which drives game clients). Both halves are gitignored deny-all with a named allowlist for their launcher, docs and `dev-plugins/` source; the rig root itself carries `CLAUDE.md`, `rig-lock.ps1` and `session.lock.template`. See "Tool: TestRig for driving the game" below.
...
The `README.md` and `RESEARCH.md` rows are scoped "each mod's folder root" because that is where nearly all of them live, but two committed non-mod tools carry the same pair for the same reason and follow the same rules: `TestRig/ClientRig/` (operating manual plus durable internals for the client rig) and each `dev-plugins/<Name>/` that needs one, such as `TestRig/DedicatedServer/dev-plugins/ScenarioRunner/README.md`. They are developer tooling, not mods, so no `About.xml` mirroring, no `CHANGELOG.md`, no `PLAYTEST.md` and no release tag applies to them; everything else in the table's Contents column does. Neither is published to the documentation site (see "Things that do NOT go in `Web/`").
...
Tier 3 is free to edit but not unowned: it belongs to whoever holds the rig session lock (see `TestRig/session.lock.template`). Do not clobber another session's save tree or instance.
...
The rig is a shared single-instance resource: every agent on this machine contends for one install. Before driving either half you MUST acquire the rig session lock (`dedicated-server.ps1 -Lock -Purpose "<reason>"`, or the same flag on `client-rig.ps1`) and pass `-As <id>` on every mutating command. One lock covers both halves. It spans a whole test session across many start/stop cycles, expires on a timer so an idle agent does not starve others, and stays live while the rig is busy (a player connected to the server, or any client-rig instance running). Breaking another session's live lock is `-BreakLock`, not `-Force`, and is human-gated: never do it without the user's explicit say-so. The complete, authoritative rules live at `TestRig/session.lock.template`; the shared implementation both launchers dot-source is `TestRig/rig-lock.ps1`. Read the rules before figuring out how to drive either half.
