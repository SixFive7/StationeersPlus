...
The decompile-content curation hook fires on five things: a Read of any file ending in `.decompiled.cs` anywhere in the tree (the suffix safety net), a Read or Glob under `.work/decomp/` (the canonical output path), a Glob naming `rocketstation_Data/Managed/` (a direct DLL listing; Read cannot serve here because the Read tool rejects binaries pre-flight, so its `PostToolUse` never fires), a Bash command that invokes a decompiler (`ilspycmd`, `ICSharpCode.Decompiler`), and a Bash command whose text contains a decompile path or suffix (`.work/decomp/`, `*.decompiled.cs`, `rocketstation_Data/Managed/`). That last case is what catches inspection via `cat`, `grep`, `rg`, `head`, `tail`, `xxd`, `strings` and anything else, without enumerating each tool. All five inject the same Rule 2 reminder pointing at `Research/WORKFLOW.md`.

The hook script decides all five for itself by reading `tool_input` from stdin. The `if` rules in `.claude/settings.json` are only a cheap pre-filter, and that split is not optional: Bash `if` rules fail open, so when the harness cannot parse a command (a loop, a heredoc) every Bash `if` rule fires regardless of content. Seven Bash rules used to mean seven copies of the reminder on a `for` loop that had never been near a decompile. The Bash registration is one entry with no `if` at all now, and the path tools carry one `*decomp*` rule each, which also ends the double-fire two overlapping Read rules produced on the canonical path. Grep is intentionally not hooked: per the Claude Code source (`GrepTool.preparePermissionMatcher`), Grep `if` rules match against the search regex argument, not the path or glob, so a path-shaped `if` rule on Grep can never fire for normal usage. Reading derived `.cs` content from anywhere else (a Downloads folder, an ad-hoc workspace, any path outside `.work/decomp/`) bypasses curation and is forbidden.
