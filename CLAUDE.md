...
- `TestRig/` is the test rig that drives Stationeers: one launcher (`TestRig/testrig.ps1`), one session lock, two halves (`TestRig/DedicatedServer/`, a self-contained headless server install, and `TestRig/ClientRig/`, N driven game clients running the `ClientDriver` plugin), plus the playtest harness under `TestRig/playtest/`. The whole tree is gitignored deny-all with a named allowlist for the launcher, its libraries and test suites, the four docs (`CLAUDE.md`, `MANUAL.md`, `RESEARCH.md`, `playtest/CLAUDE.md`) and `dev-plugins/` source. See "Tool: TestRig for driving the game" below.
...
|---|---|---|---|
| `README.md` | Each mod's folder root (committed); also `TestRig/ClientRig/` and each `dev-plugins/<Name>/` | End users (GitHub readers, Workshop subscribers) | Feature overview, installation, settings tables, issue reporting, credits, license. Source of truth for user-facing content, mirrored into `About.xml`. |
...
Tier 3 is free to edit but not unowned: it belongs to whoever holds the rig session lock (see `TestRig/CLAUDE.md`). Do not clobber another session's save tree or instance. **A dedicated-server world is also session-scoped**: one that appears after the session's first mutating command is deleted when that session ends, so stage a save you want to keep BEFORE taking the lock or before that first command.

The launcher has no copy-in or wipe verb for saves; copy save trees in directly with the standard file tools. `testrig start -Target server -New <Map>` remains the way to get a brand-new world when no source is provided.
...
`TestRig/` at the repo root is the test rig. **One launcher, `TestRig/testrig.ps1`, one session lock, one system**, with two halves and a playtest harness inside it:

```
testrig.ps1 <verb> [-Target all|server|clients|<instance>] [options]
```
...
- The **client** half (`TestRig/ClientRig/`) creates and drives N isolated game clients over HTTP through the `ClientDriver` BepInEx plugin.
- The **playtest harness** (`TestRig/playtest/`) runs a mod's in-game checks against the client half with nobody at the keyboard, and reports pass, fail or inconclusive per check. Asked to run a mod's playtests? Run the harness rather than hand-rolling the rig sequence.

Four documents, and only the first two auto-load: `TestRig/CLAUDE.md` (the rules: the lock, the save tiers, state hygiene, the safety rules; **read it before running anything under `TestRig/`**), `TestRig/playtest/CLAUDE.md` (harness work), `TestRig/MANUAL.md` (the operating reference: every verb, the working sequences, the `ClientDriver` endpoint catalogue) and `TestRig/RESEARCH.md` (durable internals). Running `testrig.ps1` with no verb prints the whole surface, and where a verb does not apply to a target the launcher refuses and names one that works.
...
The rig is a shared single-instance resource: every agent on this machine contends for one install, because the halves share the developer's one game install and per-Windows-user Unity state (`PlayerCookie-v2.xml`, the `HKCU\Software\Rocketwerkz\rocketstation` PlayerPrefs key) with each other and with the developer's own client. Before driving it you MUST acquire the session lock (`testrig lock -Purpose "<reason>"`) and pass `-As <id>` on every mutating command. It spans a whole test session across many start/stop cycles, expires on a heartbeat timer so an idle agent does not starve others, stays live while the rig is busy (a player connected to the server, or any client instance running) and is reclaimable past an absolute idle ceiling even then. Breaking another session's live lock is `-BreakLock`, not `-Force`, and is human-gated: never do it without the user's explicit say-so. **Releasing the lock restores the rig, and acquiring one restores it again as a backstop**, so your own leftovers, including a world your session created, do not survive your release unless you pass `-KeepState`. Rules: `TestRig/CLAUDE.md`; implementation: `TestRig/rig-lock.ps1` and `TestRig/rig-reset.ps1`.

The whole tree is gitignored deny-all with a named allowlist (see the `TestRig` block in `.gitignore`): the launcher, its libraries under `lib/`, the lock and reset machinery, the four test suites, the four docs, and source under `dev-plugins/`. Deny-all is deliberate, because routine rig actions drop artifacts into the folder (`snapshot -OutFile before.json`, `/screenshot?path=`) that must never be committable by accident.

Both halves keep their developer plugins under `dev-plugins/<Name>/<Name>.sln` with source in `dev-plugins/<Name>/<Name>/`. `ScenarioRunner` (`TestRig/DedicatedServer/dev-plugins/ScenarioRunner/`) probes the dedicated server from inside; `ClientDriver` (`TestRig/ClientRig/dev-plugins/ClientDriver/`) is the control plane inside each game client. Neither ships to the Workshop.

The launcher reads `<StationeersPath>` from `Directory.Build.props`, `STEAMCMD_PATH` and `STATIONEERS_CLIENTRIG_ROOT` from the environment (set per `DEV.md`). It contains no developer-specific paths.
