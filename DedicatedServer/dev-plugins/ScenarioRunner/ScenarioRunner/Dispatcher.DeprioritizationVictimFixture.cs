...
{
    // Scenario: pgp-deprioritization-victim-fixture
    //
    // Synthetic-fixture proof of PowerGridPlus's deprioritization-victim selection policy (POWER.md
    // 8.3 / 8.3.3): tier-major best-fit-decreasing. The policy, as shipped in
    // PowerAllocator.SelectDeprioritizationVictims:
    //
    //   1. Tiers go priority ASC (lowest tier is deprioritized first); selection moves to the next
    ...
    //      quantised claim covers D alone, deprioritize the SMALLEST such claim (tie: lowest
    //      ReferenceId) and stop; else deprioritize the LARGEST claim (tie: lowest ReferenceId),
    //      subtract it, repeat within the tier.
    //   3. Step-up candidates are never deprioritized; claims floor to whole Watts ((int)Math.Floor);
    ...
    // fixture reflection-resolves PowerGridPlus.PowerAllocator.SelectDeprioritizationVictims
    ...
    internal static partial class Dispatcher
    {
        ...
        private static void Scenario_PgpDeprioritizedVictimFixture()
        {
            if (!RequireModAssembly(PGP_ASSEMBLY, "pgp-deprioritization-victim-fixture")) return;
            if (_svfFired) return;
            _svfFired = true;
            ...
            try
            {
                ...
                var select = allocatorType?.GetMethod("SelectDeprioritizationVictims",
                    BindingFlags.NonPublic | BindingFlags.Static);
                if (select == null)
                {
                    _log?.LogError("[ScenarioRunner] SVF FAIL: PowerGridPlus.PowerAllocator.SelectDeprioritizationVictims not resolvable (renamed or removed?).");
                    fail++;
                    return;
                }
                ...
                {
                    _log?.LogError($"[ScenarioRunner] SVF FAIL: SelectDeprioritizationVictims signature changed (params={pars.Length}); fixture expects (IReadOnlyList<(long,int,float,bool)>, float).");
                    fail++;
                    return;
                }
                ...
                // device, one victim (the old flat walk deprioritized 500 then 1000).
                ...
                // P5: largest tie breaks to the lowest ReferenceId; both deprioritized, lower first.
                ...
                // P11: a deficit at or under the allocator's Eps tolerance (0.01 W) deprioritizes
                ...
                // is selected (never an under-selection that would leave Unmet > Eps); tie -> lowest RefId.
                ...
                // P13: a sub-Watt claim (0.5 quantises to 0) can never cover, is deprioritized last in
                // the exhaustion path, and cannot loop the selector.
