...
| `pgp-stationpedia-page-probe` | One-shot. Reads `Stationpedia._linkIdLookup` via reflection and asserts the `LogicTypeLogicPassthroughMode` entry is present (the page registered by PGP's `StationpediaPopulateLogicVariablesPatch` postfix on every `Stationpedia.PopulateLogicVariables`). On PASS, logs the page's `Key` / `Title` / `TextLen` plus a 200-char head of `Text`. The description-footer half of PGP's Stationpedia work (`Localization.GetThingDescription` postfix) is covered separately by `pgp-rate-cap-probe`, not duplicated here. Managed-state only; worker-safe. |
