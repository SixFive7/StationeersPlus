...
`stop --target server --save-name <name>` did not follow until 2026-08-16, and the gap was data loss rather than untidiness. Its guard still required a live wrapper, because that is what writing to the stdin control file needed, so an **orphaned** server handed a save name had the name ignored without the save being attempted and the world killed under it: the one path on which a caller explicitly asked for something to be preserved. What it tests now is whether a channel exists at all, which is the same question `TeardownAsync` asks. The wrapper first, because it is free to test and the answer is the same either way; otherwise a short `/status` probe, which asks ABOUT the plane rather than submitting the save into silence and spending the whole submission budget to learn the same thing. Only when neither answers is the name reported as ignored, and that report is the half that must not soften: a save that cannot be delivered has to be said out loud. It is now said in the nothing-running case too, which used to return early and say nothing about the name at all.
