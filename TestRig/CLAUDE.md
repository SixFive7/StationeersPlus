...
- **Releasing the lock RESTORES the rig, and acquiring one restores it again as a backstop.** The release is where the guarantee is earned (you clean up your own mess while you still own the rig); the acquisition covers the session that crashed and never reached its release. See "State hygiene" below; the short version is that a restore is refused while the rig is in use, never fires when you re-assert a lock you already hold, and `-KeepState` opts out loudly at either end.
...
- **Two timers, and they answer different questions.** `ttl_minutes` (default 10) is a liveness heartbeat: is anyone using the rig. `idle_ceiling_minutes` (default 60) is an absolute idle ceiling: is the OWNER still there. Refresh (`-RefreshLock -As <id>`, or any mutating command) about once a minute ONLY while actively driving a test. Never refresh to hold the rig for an absent human, and never spawn a background refresher.
- **A busy rig stays live regardless of the heartbeat, but NOT past the idle ceiling.** Busy means a player is connected to the running dedicated server, or at least one client-rig instance process is alive. Below the ceiling that keeps your live test safe in the gap between two of your own commands. Past it, the lock is reclaimable busy or not, because otherwise one forgotten instance lets a dead agent hold the whole rig forever: a hung agent must delay other agents, not block them. Only your own commands move the ceiling's clock; the busy self-renew moves the heartbeat and nothing else. `-Status` prints the countdown, and a reclaim past the ceiling is loud and names what it took the rig from. It is NOT a `-BreakLock`, which still applies only to a genuinely live lock and stays human-gated.
- **A crash is detected, not guessed.** `TestRig/session.dirty` is written durably before a session's first mutating action and cleared only by a completed restore, so a rig left dirty by a kill, a bugcheck or a power cut is restored at the next acquisition. It records the OS boot identity as well as the pid, because a pid alone means nothing across a reboot. `-Status` shows `rig state: clean` or `DIRTY`.
- The reported busy reason names what is actually happening (how many instances, which one is HOSTING, how many clients are connected to it) because that text is what a human reads when deciding whether to authorise a `-BreakLock`. A pid file alone is not proof: the launcher checks the process really is the game, so a recycled process id cannot pin the lock live forever.
...
## State hygiene: the rig is restored at both ends of a session

**Releasing the lock RESTORES the rig; acquiring one restores it again as a backstop.** The point is that a playtest cannot suddenly run badly because of garbage an unrelated playtest left behind, and that the guarantee holds by construction rather than by a rule somebody remembers. The lock is the only mandatory choke point that already exists and is already enforced in code, so an agent cannot get the rig without getting it clean and cannot route around it. Implementation: `TestRig/rig-reset.ps1`, dot-sourced by both launchers next to `rig-lock.ps1`. Full rules: `TestRig/session.lock.template`.

Restoring at BOTH ends is deliberate and is not redundancy for its own sake: the release is where the guarantee is earned (the agent that made the mess pays for it, while it still owns the rig and the rig is provably idle), and the acquisition covers the one case a release can never cover, a session that crashed or lost the machine to a reboot. Restores are idempotent, so the pair costs nothing and leaves no gap. `TestRig/session.dirty` (written durably before a session's first mutating action, cleared only by a completed restore, carrying the OS boot identity as well as the pid) is how acquisition knows.

**What "clean" means is captured, not hardcoded.** `-CaptureBaseline -As <id>` on either launcher declares the rig as it stands to be the definition of clean, writing `TestRig/baseline/` (gitignored). Config files are stored by content and restored exactly; deployed plugins and seeded mods are recorded but NEVER restored (that would undo a deliberate deploy) and instead make the baseline stale; dedicated-server worlds are recorded by name, and a world that is not in the baseline is deleted at the session boundary while baseline worlds are kept. A stale baseline (game updated, instance added, plugin rebuilt) is a loud warning naming `-CaptureBaseline`, never a silent fallback, and never blocks the lock. Without a baseline the behaviour is exactly the pre-baseline behaviour, and every reset says so.

**The reset surface is an ALLOW-LIST.** It only touches the classes above and its own hardcoded targets, so a deliberate instance-scoped change anywhere else in a tree (a real, never-hard-linked assembly dropped into one instance's own `rocketstation_Data\Managed\`, say) survives every restore and is never reported as drift. That is a permanent property of the instance, not session garbage.

**You choose how long your changes last**, and there are exactly three answers: they survive your own start/stop cycles and end when you release (default); they survive into the next session with `-Unlock -KeepState`, which leaves the dirty marker set so that session cleans up instead; or they become permanent with `-CaptureBaseline`.
...
| `TestRig/` itself | `CLAUDE.md`, `rig-lock.ps1`, `rig-lock.tests.ps1`, `rig-reset.ps1`, `rig-reset.tests.ps1`, `session.lock.template` | the live session state is all ignored: `session.lock`, `session.dirty` (the crash marker), `session.state.json` (the shared-state snapshot) and `baseline/` (the captured clean state, including its stored config content) |
