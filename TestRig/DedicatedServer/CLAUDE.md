...
1. Developer drops the source save under, for example, `%USERPROFILE%\Downloads\<Something>\`. The source is usually a single `.save` ZIP file, sometimes a folder containing that `.save` plus sibling `autosave/` / `manualsave/` / `quicksave/` directories.
2. Agent stages it under `TestRig/DedicatedServer/data/saves/<SaveName>/` in the **mandatory format below**.
...
- **Source-shape recipes** (assume source at `$src`, destination `$dest = TestRig/DedicatedServer/data/saves/<SaveName>/`):
...
- **A NEW lock resets this half's between-session state**, so a probe cannot fire inside somebody else's test. Cleared: the ScenarioRunner `Scenario` value (blanked, the rest of `net.scenariorunner.cfg` untouched), `install/BepInEx/scenariorunner/requests/` and `give/`, `install/BepInEx/inspector/requests/` and `snapshots/`, `data/setting.xml` (it carries a `SavePath` that stopped existing at the `TestRig/` restructure plus `UseSteamP2P=true`; `-Start` passes every flag it needs), and stale `server.pid` / `host.pid` / `control.cmd`. **Kept:** `data/saves/` (count and total size are reported instead, since there is no retention policy anywhere), `data/mods/`, `install/modconfig.xml`, the deployed plugin DLLs, and every other `install/BepInEx/config/*.cfg` (ones that changed since the last reset are reported, because the rig-owned versus mod-owned split is undecided). It is refused while the server process is alive, never fires when you re-assert a lock you already hold, and `-Lock -KeepState` opts out loudly. **It resets between SESSIONS: two unrelated tests under one lock get no reset between them.** Detail: `TestRig/CLAUDE.md`, "State hygiene".
...
The launcher lives next to this file at `TestRig/DedicatedServer/dedicated-server.ps1`. It reads:
...
```
TestRig/DedicatedServer/dedicated-server.ps1 -Bootstrap
```

1. Validate `Directory.Build.props` `<StationeersPath>` and `$env:STEAMCMD_PATH`.
2. Run SteamCMD with `+force_install_dir` pointed at `TestRig/DedicatedServer/install/`, install Steam app `600760` (`Stationeers Dedicated Server`) anonymously.
...
```
TestRig/DedicatedServer/dedicated-server.ps1 -SyncMods [-FromModConfig <path>]
```
...
- Recursively copy the source folder's contents into `TestRig/DedicatedServer/data/mods/<DestinationName>/`.

Then writes a baked `TestRig/DedicatedServer/install/modconfig.xml` with one `<Local Enabled="true"><Path Value="<DestinationName>" /></Local>` per copied mod, plus `<Core Enabled="true">`. This replicates Export Mod Package without driving the client UI.
...
The local mods directory is `TestRig/DedicatedServer/data/mods/` (under SavePath), NOT `TestRig/DedicatedServer/install/mods/`. The full mechanism is documented in `Research/Workflows/StationeersLaunchPadDedicatedServer.md`.

### Deploy mods built from this repo

```
TestRig/DedicatedServer/dedicated-server.ps1 -DeployMods [-Mod <name>] [-Configuration Release|Debug]
```

Copies built mod DLLs from `Mods/<X>/<X>/bin/<Configuration>/<X>.dll` into `TestRig/DedicatedServer/install/BepInEx/plugins/<X>/<X>.dll`. Without `-Mod`, every mod under `Mods/` (except `Template`) is deployed. Configuration defaults to `Release`.
...
```
TestRig/DedicatedServer/dedicated-server.ps1 -Start -Load <SaveName> -Map <Map>
TestRig/DedicatedServer/dedicated-server.ps1 -Start -New <Map>
```
...
```
TestRig/DedicatedServer/dedicated-server.ps1 -Status
```
...
```
TestRig/DedicatedServer/dedicated-server.ps1 -Logs [-Tail <N>] [-Grep <regex>]
```
...
```
TestRig/DedicatedServer/dedicated-server.ps1 -Save -Name <SaveName> [-WaitSeconds <N>]
```
...
```
TestRig/DedicatedServer/dedicated-server.ps1 -SendCommand -Command '<text>'
```
...
```
TestRig/DedicatedServer/dedicated-server.ps1 -Stop [-SaveAs <SaveName>] [-TimeoutSeconds <N>]
```
...
```
dotnet build TestRig/DedicatedServer/dev-plugins/ScenarioRunner/ScenarioRunner.sln -c Release
TestRig/DedicatedServer/dedicated-server.ps1 -DeployMods -As <id> -Mod ScenarioRunner -Configuration Release
```

The launcher detects a `TestRig/DedicatedServer/dev-plugins/<X>/` target and mirrors the built DLL plus the `About/` folder into `TestRig/DedicatedServer/data/mods/Local_<X>/` (the StationeersLaunchPad load path), then appends a matching `<Local Enabled="true">` entry to `TestRig/DedicatedServer/install/modconfig.xml` if one is not already present. Re-running is idempotent. No hand-edit of `modconfig.xml` or manual file copy is required. The same launcher invocation against a `Mods/<X>/` or `Plans/<X>/` target keeps the pre-existing behaviour of writing the DLL to `install/BepInEx/plugins/<X>/<X>.dll`.
...
TestRig/DedicatedServer/dedicated-server.ps1 -Start -As <id> -Load <save> -Map <Map>
grep "\[ScenarioRunner\]" TestRig/DedicatedServer/install/BepInEx/LogOutput.log
```

Mod-specific scenarios are prefixed (e.g. `pgp-` for PowerGridPlus); they no-op with a warning if the named mod is absent. Add new scenarios by editing `TestRig/DedicatedServer/dev-plugins/ScenarioRunner/ScenarioRunner/Dispatcher.cs`, adding a `case` to the switch and a `Scenario_*` method, then rebuilding. Full scenario catalogue and authoring guide in `TestRig/DedicatedServer/dev-plugins/ScenarioRunner/README.md`.
...
5. `TestRig/DedicatedServer/dedicated-server.ps1 -Start -As <id> -New <Map>`, OR ask the developer for a save name and use `-Start -As <id> -Load <save> -Map <Map>`. The launcher returns within ~5 s of the server registering its PID.
