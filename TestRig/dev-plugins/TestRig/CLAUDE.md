...
The id rule is not cosmetic. `Client.connectionId` is a `long` RakNet id in the 10^17 range, and emitting it as a raw JSON number made `System.Text.Json` throw on the **whole** `/status` payload on the launcher side, where the reader returned null on a parse failure. One oversized field made the entire endpoint unreadable, and the harness concluded the joiner had never arrived. Both ids on a roster row are strings now, and the reader names the offending field instead of erasing the response. **Never emit a game id as a bare JSON number without checking what the launcher's Contracts record types it as.**
