...
**2. `connectionId` took the whole response down.** `Client.connectionId` is a `long` RakNet connection id, and the values are enormous: 189151461494586169 and 1044835390751713754 in one measured join. Emitted as a raw JSON number it did not fit the launcher's `ConnectedClient.ConnectionId`, which was `int?`, so `System.Text.Json` threw on the **whole** `/status` payload, `RigWire.Deserialize` returned null by design, and the launcher's roster poll concluded nothing had arrived. That is what produced three attempts of `roster did not grow (0 then 0)` against a host whose own console showed the client verified, served, ready and holding the session.

**Both of those are now closed on the correct side.** `ConnectedClient.ConnectionId` is a `string`, exactly as `clientId` is and for the same reason: both ids on a row are past 2^53, both are compared rather than computed, and two ids side by side spelled two different ways is the trap. So the plugin no longer degrades anything. It emits the true value under its own name, unconditionally, and the `connectionIdString` twin the degradation needed is gone; nothing ever read it but a unit test. `RigWire.Deserialize` no longer returns null on a parse failure either, so a future disagreement of this shape names the field and the value instead of erasing the response.

The rule the whole episode leaves behind: **never emit a game id as a bare JSON number without checking what the launcher's Contracts record types it as.**
