...
- **The launcher could not deploy it.** The PowerShell launcher resolved a deploy target under `Mods/`, then `Plans/`, then `DedicatedServer/dev-plugins/` and `ClientRig/dev-plugins/`, and `TestRig/dev-plugins/` was in none of them; its client half additionally hardcoded `ClientDriver.sln` / `ClientDriver.dll`. `testrig.exe` searches `TestRig/dev-plugins/` ahead of both per-half folders and resolves the control plugin by name rather than hardcoding one, so this no longer holds. Both instances and the dedicated server carry `TestRig.dll` today.
