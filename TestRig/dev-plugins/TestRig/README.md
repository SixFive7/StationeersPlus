...
Deliberately. Building alongside the two plugins it replaces is the whole point until parity is proven.

- **The launcher cannot deploy it.** `TestRig/lib/common.ps1` resolves a deploy target under `Mods/`, then `Plans/`, then `DedicatedServer/dev-plugins/` and `ClientRig/dev-plugins/`. `TestRig/dev-plugins/` is not in that list. The client half additionally hardcodes `ClientDriver.sln` / `ClientDriver.dll` in `lib/client.ps1`, and copies the plugin at instance-create time.
...
- **This assembly has not itself been run.** No game process has been started with `TestRig.dll`. The measurements above were taken with separate instrumented probes against the real dedicated server on 0.2.6428.27798, and the pump design follows from them, but the claim "this plugin works" is not among the things measured. Everything else here is from source or from the build.
