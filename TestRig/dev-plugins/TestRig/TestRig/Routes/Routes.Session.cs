    ...
    internal static partial class Router
    {
        ...
        private static string InstanceRoute(IDictionary body)
        {
            ...
            return new Json.Obj()
                ...
                // Which of the two processes this is, and which main-thread marshal it chose.
                // /instance is the "what am I talking to" endpoint, so the host profile belongs
                // here as much as on /status, and this one answers before a world exists.
        ...
        ///     3. Redirecting INTO the developer's REAL user-data folder is refused, FULL STOP.
        ///        ClientDriver let <c>force=true</c> skip that check. The override is gone: this
        ///        endpoint can be reached by raw curl from anything on the machine, so a rule in a
        ///        document ("never pass force=true", which three documents had to say) is not a
        ///        guard, and the launcher cannot guard a request that does not go through it. The
        ///        comparand is <see cref="RealUserDataPath"/>, computed without asking the game,
        ///        for the reason spelled out on that method. Passing <c>force</c> now produces a
        ///        400 naming the removal rather than being ignored, because silently dropping a
        ///        parameter the caller believed in is how a caller ends up trusting a result it
        ///        should not.
        /// </summary>
        private static HttpResponse SavePath(IDictionary body)
        {
            // Refused, not ignored. Same shape as /dlc/remove's nine grant-shaped fields: a
            // request that names an override this endpoint no longer honours is answered rather
            // than quietly reinterpreted.
            if (Json.Has(body, "force"))
                return HttpResponse.Error(
                    "'force' is not a parameter of this endpoint any more, and nothing was changed. It " +
                    "used to skip the tier-1 check that keeps a driven session out of the developer's " +
                    "real save folder. That folder is now unreachable through this endpoint by " +
                    "construction. Remove the parameter and point the path at the instance's own save " +
                    "root; GET /savepath reports both that root and the folder that is off limits.", 400);
            ...
            // THE TIER-1 GATE. Unconditional. There is no override parameter and adding one back
            // would defeat the whole guard.
            //
            // ClientDriver had force=true here. The rig's own rules then had to say, in three
            // separate documents, "never pass force=true", which is a rule an agent has to have
            // read to obey, and this endpoint is reachable by raw curl from anything on the
            // machine. The launcher cannot be the guard for a request that never goes through the
            // launcher. So the guard is here and it cannot be talked out of.
            //
            ...
            if (IsInside(resolved, realUserData))
                return HttpResponse.Error(
                    ...
                    "session AWAY from that folder is the entire purpose of this endpoint. There is no " +
                    "override: the previous force=true parameter was removed so that folder is " +
                    "unreachable by construction rather than by a rule someone has to have read. " +
                    "Point the path at the instance's own save root instead. Nothing was changed.", 409);
