...
namespace TestRig.Scenarios
{
    ...
    internal static partial class Dispatcher
    {
        ...
        {
            // Transients first, so a scenario invoked over HTTP is not starved by a long armed
            // set, and so its budget is spent on the same tick the caller is counting.
            TickTransients();
        ...
        private static void TickOne(string scenario)
        {
            ...
            switch (scenario)
            {
                ...
                default:
                    // Recorded, not just warned once.
                    //
                    // The single warning at _ticksSeen == _delayTicks was one of the ways a probe
                    // could look armed and produce nothing: miss that line in a multi-megabyte log
                    // and a typo is indistinguishable from a scenario that ran and found nothing.
                    // NoteUnknown makes it a standing fact that GET /scenarios reports.
                    NoteUnknown(scenario);
        ...
        private static bool RequireModAssembly(string assemblyName, string scenarioId)
        {
            ...
            // Recorded as well as warned. The warning fires on exactly one tick and then the
            // scenario returns false in silence for the rest of the session, which reads
            // identically to a probe that ran and found nothing. GET /scenarios reports this.
            NoteBlockedByAssembly(scenarioId, assemblyName);
            if (_ticksSeen == _delayTicks)
