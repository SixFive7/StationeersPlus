...
namespace TestRig
{
    ...
    internal static class Json
    {
        ...
        public sealed class Obj
        {
            ...
            /// <summary>
            ///     An array of already-encoded JSON fragments. The routes that build a list of
            ///     sub-objects were doing this by hand with a StringBuilder in five places; this
            ///     is the same thing named once. Nothing here re-escapes: the caller has already
            ///     produced JSON.
            /// </summary>
            public Obj RawArray(string key, IEnumerable<string> encodedValues)
            {
                var sb = new StringBuilder("[");
                bool first = true;
                if (encodedValues != null)
                {
                    foreach (var v in encodedValues)
                    {
                        if (!first) sb.Append(',');
                        first = false;
                        sb.Append(v ?? "null");
