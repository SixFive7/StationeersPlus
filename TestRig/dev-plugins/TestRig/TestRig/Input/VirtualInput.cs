...
namespace TestRig
{
    ...
    internal static class VirtualInput
    {
        ...
        // ---- read-back ------------------------------------------------------
        //
        // "The driver applied the override" and "the game read the override" are
        // different claims, and only the second one is worth anything to a caller.
        // The previous shape of this class could only report the first, so
        // /input/key answered {"ok":true,"settled":true} for a keypress the game
        // never observed, and a whole acceptance test was built on that answer and
        // came out confidently wrong.
        //
        // These counters are incremented at the exact moment a synthetic value is
        // handed back to a caller, so a non-zero count is proof of delivery rather
        // than proof of intent. Only keys the driver has actually injected are
        // tracked, which keeps the bookkeeping to a HashSet probe on the hot path.
        internal sealed class Observation
        {
            public long GetKey;
            public long GetKeyDown;
            public long GetKeyUp;
            public long Total { get { return GetKey + GetKeyDown + GetKeyUp; } }
        }

        // Only ever written when a synthetic value is actually returned, which can
        // only happen for a key the driver injected, so an untouched key costs
        // nothing on the hot path.
        private static readonly Dictionary<KeyCode, Observation> _observed =
            new Dictionary<KeyCode, Observation>();

        private static Observation ObservationFor(KeyCode key)
        {
            Observation o;
            if (!_observed.TryGetValue(key, out o)) { o = new Observation(); _observed[key] = o; }
            return o;
        }

        /// <summary>Clears the delivery record for one key. Call before scheduling.</summary>
        internal static void ResetObservation(KeyCode key)
        {
            lock (_gate) { _observed[key] = new Observation(); }
        }

        /// <summary>Delivery record for one key since the last reset.</summary>
        internal static Observation GetObservation(KeyCode key)
        {
            lock (_gate)
            {
                Observation o;
                if (!_observed.TryGetValue(key, out o)) return new Observation();
                return new Observation { GetKey = o.GetKey, GetKeyDown = o.GetKeyDown, GetKeyUp = o.GetKeyUp };
            }
        }

        /// <summary>Times a synthetic wheel value was handed to a caller since the last reset.</summary>
        internal static long ScrollReads;

        internal static void ResetScrollObservation()
        {
            lock (_gate) { ScrollReads = 0; }
        }

        /// <summary>Times a synthetic mouse position was handed to a caller since the last reset.</summary>
        internal static long MousePositionReads;

        internal static void ResetMousePositionObservation()
        {
            lock (_gate) { MousePositionReads = 0; }
        }
