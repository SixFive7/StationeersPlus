...
/// <remarks>
/// <para>
/// Twenty-one rows, each carrying what was attempted, why this target cannot do it, a
/// command that does work, and where the durable explanation lives. All five are mandatory
/// because <see cref="Refusal"/> makes them so: a refusal without a working alternative is
/// the failure the matrix exists to prevent.
/// </para>
/// <para>
...
public static class RefusalMatrix
{
    ...
    [
        ...
        // 2
        new("call", "server", "", new Refusal(
            What: "'call' against the dedicated server",
            Why: "'call' sends an HTTP request to the ClientDriver control plane running INSIDE a game client, and reads the parsed answer back. The dedicated server has no such plane: it is driven through its stdin by a wrapper process polling a control file, which is fire and forget. It also has no player character at all (it runs with IsBatchMode true, so CreateCharacterAndTakeControl never runs and LocalClientId stays 0), so every /player, /inventory, /cursor and /input path has nothing to act on.",
        ...
        // 3
        new("call", "all", "", new Refusal(
            What: "'call' fanned out across both halves",
            Why: "'call' cannot fan out across both halves: --target all includes the dedicated server, which has no HTTP control plane. Fanning out over the instances alone is a different command from talking to the server, on purpose, because one returns a parsed answer per instance and the other is fire and forget.",
        ...
        new("snapshot", "server", "", new Refusal(
            ...
            Why: "'snapshot' fetches /status from each instance's control plane. The dedicated server has no control plane, so there is no equivalent blob to fetch. What it can answer about itself comes from its own state files and its log.",
        ...
        new("snapshot", "all", "", new Refusal(
            ...
            Why: "'snapshot' is a control-plane fan-out and --target all includes the dedicated server, which has none. Mixing a half that answers with a half that cannot would produce a file whose shape depends on what happened to be running.",
        ...
        new("wait", "server", "client-stage", new Refusal(
            What: "'wait' for a client readiness stage on the dedicated server",
            Why: "the readiness stages 'ping', 'modsLoaded' and 'menu' are client-instance states. A dedicated server has no control plane to ping, and never has a menu at all: it enters its world from the command line, so the only readiness question about it is whether that world is loaded and the simulation is ticking.",
        ...
        // 22. Last, and the only '*' row, so a specific row always wins the scan.
    ...
    public static void Assert(string verb, ResolvedTarget resolved, RefusalInputs inputs)
    {
        ...
        switch (verb)
        {
            ...
            case "call":
                if (resolved.Kind == TargetKind.Server) throw Deny("call", "server", target: shown);
                if (resolved.Kind == TargetKind.All) throw Deny("call", "all", target: shown);
                break;
    ...
    /// <summary>The three readiness stages that only a client instance can be in.</summary>
    public static bool IsClientStage(string stage) =>
        stage is "ping" or "modsLoaded" or "menu";
